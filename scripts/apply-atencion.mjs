// Aplica sql/migrate-atencion.sql a las bases de BDI y ZATTIA del monitor.
// Lee DATABASE_URL_BDI / DATABASE_URL_ZATTIA del .env (una o ambas).
// Uso: node scripts/apply-atencion.mjs
...
const sql = readFileSync('sql/migrate-atencion.sql', 'utf8')
...
for (const [nombre, url] of targets) {
  ...
  try {
    ...
    const r = await client.query('select count(*)::int as n from atencion')
    console.log(`✓ ${nombre} (${cfg.host}): tabla atencion lista — ${r.rows[0].n} filas`)
  } catch (e) {
