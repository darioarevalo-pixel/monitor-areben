// Aplica sql/migrate-meta-reglas.sql a la base de BDI del monitor.
// Lee DATABASE_URL_BDI del .env.
// Uso: node scripts/apply-meta-reglas.mjs
//
// ⚠️ Va a UNA SOLA base, igual que apply-meta-planes.mjs y apply-meta-snapshot.mjs, y por el mismo
// motivo: las cuentas publicitarias son compartidas entre líneas. El detalle está arriba del SQL.
//
// Idempotente (`create table if not exists` + `create index if not exists`): seguro re-correr.
// Crea tres tablas NUEVAS, no toca ninguna existente ni migra datos.
...
const sql = readFileSync('sql/migrate-meta-reglas.sql', 'utf8')
const TABLAS = ['meta_ads_regla', 'meta_ads_hallazgo', 'meta_ads_umbral']
...
try {
  ...
  for (const t of TABLAS) {
    ...
    console.log(`  ${t.padEnd(28)} ${String(r.rows[0].n).padStart(5)} filas`)
  }
} catch (e) {
