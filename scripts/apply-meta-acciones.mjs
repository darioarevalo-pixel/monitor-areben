// Aplica sql/migrate-meta-acciones.sql a la base de BDI del monitor.
// Lee DATABASE_URL_BDI del .env.
// Uso: node scripts/apply-meta-acciones.mjs
//
// ⚠️ Va a UNA SOLA base, igual que apply-meta-ads-linea.mjs y por el mismo motivo: la cuenta
// publicitaria es una sola para las tres líneas, así que "quién apagó qué" es un hecho único.
// Partirlo por store daría dos historias parciales de la misma cuenta. El detalle está arriba del SQL.
//
...
const sql = readFileSync('sql/migrate-meta-acciones.sql', 'utf8')
