// Aplica sql/migrate-manuales.sql a la base de BDI.
//
...
const sql = readFileSync('sql/migrate-manuales.sql', 'utf8')
...
try {
  ...
  const n = await client.query('select count(*)::int as n from manuales')
  console.log(`✓ BDI (${cfg.host}): manuales ${n.rows[0].n} filas`)
} catch (e) {
