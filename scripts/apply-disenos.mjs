// Aplica sql/migrate-disenos.sql a las bases de BDI y ZATTIA del monitor.
// Lee DATABASE_URL_BDI / DATABASE_URL_ZATTIA del .env (una o ambas).
// Uso: node scripts/apply-disenos.mjs
...
const sql = readFileSync('sql/migrate-disenos.sql', 'utf8')
...
for (const [nombre, url] of targets) {
  ...
  try {
    ...
    const r = await client.query('select count(*)::int as n from disenos')
    console.log(`✓ ${nombre} (${cfg.host}): tabla disenos lista — ${r.rows[0].n} filas`)
  } catch (e) {
