...
// Las migraciones en orden, todas idempotentes: crea la tabla y luego la extiende.
const sql = [
  'sql/migrate-devoluciones.sql',
  'sql/migrate-devoluciones-2.sql',
  ...
  'sql/migrate-reclamos-retencion-at.sql',
...
for (const [nombre, url] of targets) {
  ...
  try {
    ...
    const c = await client.query(`select column_name from information_schema.columns
      where table_name = 'devoluciones' and column_name in ('envio_nuevo_estado', 'escenario', 'retencion_respuesta', 'retencion_monto', 'cupon_estado')`)
    const faltan = ['envio_nuevo_estado', 'escenario', 'retencion_respuesta', 'retencion_monto', 'cupon_estado'].filter((k) => !c.rows.some((x) => x.column_name === k))
