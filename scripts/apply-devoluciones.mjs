...
for (const [nombre, url] of targets) {
  ...
  try {
    ...
    const c = await client.query(`select column_name from information_schema.columns
      where table_name = 'devoluciones' and column_name in ('envio_nuevo_estado', 'escenario')`)
    const faltan = ['envio_nuevo_estado', 'escenario'].filter((k) => !c.rows.some((x) => x.column_name === k))
