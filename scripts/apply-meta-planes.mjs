// Aplica sql/migrate-meta-planes.sql a la base de BDI del monitor.
// Lee DATABASE_URL_BDI del .env.
// Uso: node scripts/apply-meta-planes.mjs
//
// ⚠️ Va a UNA SOLA base, igual que apply-meta-acciones.mjs y por el mismo motivo: la cuenta
// publicitaria es una sola para las tres líneas. El detalle está arriba del SQL.
//
// Idempotente (`create table if not exists` + `create index if not exists`): seguro re-correr.
// Crea dos tablas NUEVAS, no toca ninguna existente ni migra datos.
...
const sql = readFileSync('sql/migrate-meta-planes.sql', 'utf8')
const TABLAS = ['meta_ads_plan', 'meta_ads_plan_paso']
