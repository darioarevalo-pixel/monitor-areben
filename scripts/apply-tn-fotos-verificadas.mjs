// Aplica sql/migrate-tn-fotos-verificadas.sql a las bases de BDI y ZATTIA del monitor.
// Lee DATABASE_URL_BDI / DATABASE_URL_ZATTIA del .env (una o ambas).
// Uso: node scripts/apply-tn-fotos-verificadas.mjs
...
const sql = readFileSync('sql/migrate-tn-fotos-verificadas.sql', 'utf8')
...
for (const [nombre, url] of targets) {
  ...
  try {
    ...
    const r = await client.query('select count(*)::int as n from tn_fotos_verificadas')
    console.log(`✓ ${nombre} (${cfg.host}): tabla tn_fotos_verificadas lista — ${r.rows[0].n} filas`)
  } catch (e) {
