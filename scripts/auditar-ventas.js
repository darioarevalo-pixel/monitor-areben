 ...
 *
 * Este script las separa con un dato que no depende de nuestra interpretación:
 * `items_sold`, las unidades que GN declara para la venta. Para cada venta compara TRES
 * cifras —lo que dice el Monitor hoy, lo que devuelve GN ahora, y lo que GN declara— y
 * clasifica qué pasaría si purgáramos:
 *
 ...
 *   ⚠️ PELIGRO      el espejo ya coincidía y la purga igual borraría. Señal de (b).
 *   ⚠️ GN INCOMPLETO GN devuelve MENOS unidades que las que su propia venta declara.
 *   sin dato        la venta no tiene items_sold (todo 2024): no se puede juzgar.
 *
 * La decisión de borrar o no depende de si aparecen casos PELIGRO o GN INCOMPLETO.
 * Mientras haya, no se purgan renglones.
 *
 * Uso:
...
async function main() {
  ...
  const clases = { OK: 0, ARREGLA: 0, DUPLICADO: 0, PELIGRO: 0, GN_INCOMPLETO: 0, SIN_DATO: 0, SIN_DETALLE_GN: 0 };
  ...
  for (const v of ventasEspejo) {
    ...
    if (v.items_sold == null) { clases.SIN_DATO++; continue; }

    const declara = Number(v.items_sold);
    ...
    if (uEspejo === declara) {
      // el espejo YA coincidía con GN y la purga igual borraría: no hay que tocarlo
      clases.PELIGRO++;
