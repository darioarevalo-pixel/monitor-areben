// Aplica sql/migrate-solicitudes.sql a las bases de BDI y ZATTIA del monitor.
...
//
// Después de esto va la migración de datos: node scripts/migrar-solicitudes.mjs --dry-run
...
const sql = readFileSync('sql/migrate-solicitudes.sql', 'utf8')
...
for (const [nombre, url] of targets) {
  ...
  try {
    ...
    const r = await client.query('select count(*)::int as n from solicitudes')
    console.log(`✓ ${nombre} (${cfg.host}): tabla solicitudes lista — ${r.rows[0].n} filas`)
  } catch (e) {
