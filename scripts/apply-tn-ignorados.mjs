// Aplica sql/migrate-tn-ignorados.sql a las bases de BDI y ZATTIA del monitor.
// Lee DATABASE_URL_BDI / DATABASE_URL_ZATTIA del .env (una o ambas).
// Uso: node scripts/apply-tn-ignorados.mjs
...
const sql = readFileSync('sql/migrate-tn-ignorados.sql', 'utf8')
...
for (const [nombre, url] of targets) {
  ...
  try {
    ...
    const r = await client.query('select count(*)::int as n from tn_ignorados')
    console.log(`✓ ${nombre} (${cfg.host}): tabla tn_ignorados lista — ${r.rows[0].n} filas`)
  } catch (e) {
