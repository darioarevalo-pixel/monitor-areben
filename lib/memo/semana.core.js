...
/**
 * ¿Se puede escribir ese bloque con la semana en ese estado?
 *
 * 🔴 **Cerrar congela los NÚMEROS. El acta no es un número.** Hasta el 7-sep-2026 esto no era una
 * regla: era el mismo `estaCerrado` de la pantalla apagando cuatro cosas de una sola vez (la foto,
 * las señales, los avances y el acta), y la única forma de escribir el acta de una semana cerrada
 * era un UPDATE a mano en la base. Pasó dos veces —la `w2026-08-10` quedó sin acta para siempre y
 * la `w2026-08-31` se cerró con el acta en blanco—, y lo dijo Bruno: *"por qué no se puede seguir
 * escribiendo lo de memo semanal personal de cada uno"*.
 *
 * 🔑 El corte es **qué depende de qué**. Venta, pauta, capital parado y los avances son la foto de
 * la semana: si se pueden reescribir después, el histórico deja de ser histórico. Cada casilla del
 * acta es de **una** persona, va con su firma y su hora, y **ningún número deriva de ella** —
 * cerrar la semana no puede ser lo que impida escribir lo que pasó esa semana.
 *
 * ⛔ La regla vive acá y no en el JSX porque la aplican DOS llamadores: la pantalla (que decide si
 * dibuja el textarea) y `api/_memo.js` (que decide si guarda). Escrita sólo en la pantalla, un POST
 * seguía entrando —era exactamente el caso: el candado del cierre nunca existió del lado del
 * servidor—.
 *
 * @param {'acta'|'avance'} bloque
 * @param {string} estado estado de la semana (`'abierto'` | `'cerrado'`)
 */
export function puedeEscribirBloque(bloque, estado) {
  if (bloque === 'acta') return true
