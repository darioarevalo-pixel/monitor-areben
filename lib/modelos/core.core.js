...
/**
 * ¿Esta ficha es de la marca que está mirando quien la pide?
 *
 ...
 *
 * 🔑 **Vive acá y ⛔ no en `core.ts`** desde el 3-sep-2026: la sesión de fotos elige la modelo de una
 * lista que arma **el handler**, y el handler ⛔ no puede importar TypeScript. Escribir el mismo
 * criterio dos veces sería exactamente lo que pasó con el talle. `core.ts` lo re-exporta.
 */
export function esDeLaMarca(m, marca) {
  const marcas = Array.isArray(m?.marcas) ? m.marcas : []
  return !marcas.length || marcas.includes(marca)
}

/**
 * ¿Se le puede ofrecer esta ficha a quien está cargando una sesión de fotos?
 *
 * Dos condiciones y ⛔ ninguna más: que esté **activa** —`archivada` es justamente «sale de la lista
 * de a quién llamar»— y que sea **de esa marca**. ⛔ No pide talle: una ficha sin talle igual se
 * elige, y ahí el talle se tipea en la sesión; exigirlo escondería a la modelo que está parada en
 * el estudio porque su ficha está a medio cargar.
 */
export function esElegible(m, marca) {
  return m?.estado !== 'archivada' && esDeLaMarca(m, marca)
}
