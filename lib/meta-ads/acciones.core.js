 ...
 *
 * 🔑 **`vuelta` es la pregunta de quien aprieta, y ⛔ no es `reintentable`** (12-sep-2026, P1 de
 * `PENDIENTES.md`). `reintentable` dice si el SERVIDOR puede repetir la llamada sin hacer daño;
 * `vuelta` dice si la PERSONA puede dejar las cosas como estaban. Son distintas y se contestan
 * distinto: cambiar el presupuesto es `reintentable: true` y sin embargo ⛔ no se vuelve atrás del
 * todo —el número se repone, la fase de aprendizaje ya se reinició—. Hasta hoy eso se leía recién
 * adentro del modal, o no se leía en ningún lado.
 *
...
export const ACCIONES = {
  estado: {
    ...
    campos: ['status'],
    reintentable: true,
    // Se deshace con el mismo botón, y por eso es el único que queda afuera del «⋯»: es el gesto de
    // todos los días. ⚠️ Lo que ⛔ no vuelve es lo que no entregó mientras estuvo pausado.
    vuelta: { como: 'si', texto: 'Se vuelve atrás con el mismo botón. Lo que no se mostró mientras estuvo pausado ⛔ no se recupera.' },
  },
  presupuesto: {
    sub: 'presupuesto',
    ...
    campos: ['daily_budget'],
    reintentable: true,
    // 🔴 El número se repone; la fase de aprendizaje ⛔ NO. Es el caso que obliga a que `vuelta` sea
    // un campo aparte de `reintentable`: los dos valores son opuestos sobre la misma acción.
    vuelta: { como: 'parcial', texto: 'El número se puede reponer, pero el reinicio del aprendizaje de Meta ⛔ no se deshace: los primeros días después de cada cambio rinden peor.' },
  },
  nombre: {
    ...
    // que el primero.
    reintentable: true,
    // ⚠️ El nombre viejo ⛔ no queda guardado en ninguna parte: para volver hay que saberlo. Y el
    // parte del día agrupa por nombre, así que un renombre parte la historia de ese objeto en dos.
    vuelta: { como: 'parcial', texto: 'Se puede volver a poner el nombre anterior, pero hay que saberlo: no queda guardado. Y los informes que agrupan por nombre parten la historia del objeto en dos.' },
  },
  duplicar: {
    ...
    reintentable: false,
    // 🔴 Lo único que crea objetos en Meta desde acá. La copia nace PAUSADA, así que ⛔ no gasta, pero
    // eliminarla es a mano en el Administrador de anuncios: el monitor ⛔ no elimina nada.
    vuelta: { como: 'no', texto: 'Crea objetos nuevos en Meta. Nacen pausados y ⛔ no gastan, pero el monitor ⛔ no los elimina: eso es a mano en el Administrador de anuncios.' },
  },
}
