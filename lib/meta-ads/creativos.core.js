...
export async function piezasDeCuenta(cuentaId) {
  ...
  const [baseRes, ricoRes] = await Promise.all([
    graph(`act_${cuentaId}/ads?fields=${campos},creative{thumbnail_url,effective_object_story_id,instagram_permalink_url}&limit=200`),
    graph(`act_${cuentaId}/ads?fields=id,creative{id,image_url,body,title,object_story_spec}&limit=200`),
  ])
  if (!baseRes.ok) return { ok: false, error: baseRes }
  ...
  const piezas = ((baseRes.data && baseRes.data.data) || []).map((a) => {
  ...
  return { ok: true, piezas, sinRico: ricoRes.ok ? null : ricoRes }
}
