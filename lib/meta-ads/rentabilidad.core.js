...
/**
 * **Cuál de los dos costos por compra manda: el que se MIDIÓ o el que alguien TIPEÓ.**
 *
 * # El defecto que esta función existe para cerrar
 *
 * `costoHoy` es un supuesto que se escribe a mano, y de él cuelgan `aire` y el renglón «el de hoy»
 * de la proyección. 🔴 **Un supuesto tipeado envejece y la pantalla no se entera.** Medido el
 * 30-ago-2026, con las dos fichas guardadas contra la foto de sus últimos 7 días cerrados:
 *
 * | línea | lo que la ficha afirmaba | lo medido | |
 * |---|---|---|---|
 * | BDI | $2.472 ⇒ **2,70× de aire** | $5.697 (85% del techo) ⇒ **1,17×** | afirma **2,3× de más** |
 * | Zattia | $3.069 ⇒ **1,97× de aire** | $913 (15% del techo) ⇒ **6,62×** | afirma **3,4× de menos** |
 *
 * ⇒ las dos mentían, **y para lados opuestos**: no es un sesgo que se pueda corregir con una
 * constante, es un número que dejó de ser cierto. La pantalla que le pone la vara a todo el módulo
 * decía que a BDI le sobraba aire para escalar justo la semana en que el pedido marginal salía
 * $13.388 contra un techo de $6.668.
 *
 * # Las tres decisiones
 *
 * 1. 🔑 **Manda la foto, y cuando no hay foto se dice.** ⛔ Nunca al revés: un supuesto que le gana
 *    a una medición sólo se justifica si alguien lo eligió, y para eso está el motivo escrito.
 * 2. 🔴 **`pedidos > 0` es la condición, ⛔ no `costo > 0`.** Sin pedidos el cociente da 0, y un 0
 *    acá se lee como «te sale gratis»: es el mismo cero que afirma de `semaforoPauta`. La ventana
 *    sin pedidos ⛔ no gobierna nada.
 * 3. 🔑 **`discrepa` sale SIEMPRE que existan los dos**, aunque mande la foto: es lo que la pantalla
 *    tiene para decir que el campo guardado quedó viejo. Callarlo dejaría el número tipeado
 *    envejeciendo para siempre, que es exactamente cómo llegamos acá.
 *
 * ⚠️ **Esto ⛔ no mueve el techo.** `costoMax` no depende de `costoHoy` —sale de la economía
 * unitaria y del reparto—, así que sustituir acá cambia el AIRE y la proyección, ⛔ nunca la vara.
 * Es lo que hace que se pueda sustituir sin que ninguna otra pantalla lea otro techo.
 *
 * @param medido   `{ costo, pedidos, desde, hasta }` de la foto, o `null` si no contestó
 * @param tipeado  el `costoHoy` guardado en la ficha
 * @returns `{ costo, fuente, motivo, discrepaPct }` — `fuente` es `'foto' | 'ficha' | 'ninguno'`
 */
export function costoQueManda({ medido = null, tipeado = 0 } = {}) {
  const t = Number(tipeado) > 0 ? Number(tipeado) : 0
  const m = medido && Number(medido.costo) > 0 && Number(medido.pedidos) > 0 ? medido : null
  // Se calcula antes de elegir: la discrepancia es un hecho sobre los dos números, ⛔ no sobre el
  // que ganó. Con uno solo de los dos no hay nada que comparar y va `null` — ⛔ no 0, que se leería
  // como «coinciden».
  const discrepaPct = m && t > 0 ? ((t - Number(m.costo)) / Number(m.costo)) * 100 : null

  if (m) return { costo: Number(m.costo), fuente: 'foto', motivo: '', discrepaPct }
  if (t > 0) {
    return {
      costo: t,
      fuente: 'ficha',
      motivo: medido && Number(medido.pedidos) === 0
        ? 'la foto de esta línea no tiene ningún pedido en la ventana'
        : 'todavía no hay foto cerrada de esta línea',
      discrepaPct,
    }
  }
  return { costo: 0, fuente: 'ninguno', motivo: 'no hay foto ni un costo cargado en la ficha', discrepaPct }
}
