...
/**
 * 🔴 **Lo que NO puede entrar, y por qué la puerta es el único lugar donde frenarlo.**
 *
 * El plan era dibujar el informe en un iframe con `sandbox`, y que el navegador se ocupara. Medido
 * en producción, **con `sandbox` el marco queda en blanco** —con el informe entero, con 3 KB, con
 * y sin `<style>`, por `srcdoc` y por blob— y sin `sandbox` se ve siempre. Un aislamiento que no
 * deja ver nada no es aislamiento: es la pantalla rota.
 *
 * Así que la garantía se muda de lugar. **Un informe con JavaScript no se guarda**, y eso sí se
 * puede sostener: es una condición sobre el texto, se verifica acá, y no depende de que un atributo
 * se comporte igual en cada navegador. Los informes son documentos de lectura; ninguno necesitó
 * nunca un script.
 *
 * ⚠️ Esto NO es un saneador: no limpia el HTML, lo **rechaza**. Limpiar en silencio deja al que
 * sube creyendo que subió lo que escribió.
 */
export const RIESGOS = [
  { re: /<script[\s>]/i, que: 'una etiqueta <script>' },
  { re: /\son[a-z]+\s*=\s*["'`]/i, que: 'un manejador de eventos en línea (onclick, onload, …)' },
  { re: /javascript:/i, que: 'una URL «javascript:»' },
  { re: /<iframe[\s>]/i, que: 'un <iframe> anidado' },
  { re: /<object[\s>]|<embed[\s>]/i, que: 'un <object> o un <embed>' },
]

/** Qué peligros trae este HTML. Vacío = se puede guardar. */
export function riesgosDelHtml(html) {
  const txt = String(html || '')
  return RIESGOS.filter((r) => r.re.test(txt)).map((r) => r.que)
}
...
export function validarInforme(cuerpo, { lineasValidas }) {
  ...
  // 🔴 La puerta, no el navegador. Ver `RIESGOS`.
  const riesgos = riesgosDelHtml(html)
  if (riesgos.length) {
    return {
      ok: false,
      error: `El informe trae ${riesgos.join(' y ')}. Un informe es un documento de lectura: se guarda sin JavaScript, o no se guarda.`,
    }
  }
...
export function avisosDelHtml(html) {
  const out = []
  const txt = String(html || '')
  // El `<script>` no está acá: no es un aviso, es un rechazo. Ver `RIESGOS`.
