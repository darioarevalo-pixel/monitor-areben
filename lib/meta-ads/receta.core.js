...
/**
 * Los objetivos con los que Meta todavía deja **crear** una campaña. Medidos con `validate_only`
 * el 9-ago-2026, no sacados de la documentación.
 *
 * 🔴 **`LINK_CLICKS` está retirado** —Meta contesta «Objective LINK_CLICKS is invalid»— y es el
 * objetivo de **23 de los 43 conjuntos activos** de la cuenta grande. Las campañas viejas siguen
 * andando: lo que no se puede es hacer una nueva así. Consecuencia directa: un conjunto de una
 * campaña `LINK_CLICKS` **no sirve de receta**, y hay que decirlo antes y no en el paso 1.
 */
export const OBJETIVOS_VIVOS = ['OUTCOME_SALES', 'OUTCOME_TRAFFIC', 'OUTCOME_ENGAGEMENT', 'OUTCOME_AWARENESS']

/** ¿Se puede crear una campaña nueva con este objetivo? */
export const objetivoUsable = (objetivo) => OBJETIVOS_VIVOS.includes(String(objetivo || ''))

/**
 * El cuerpo del `POST act_<id>/campaigns`.
 *
 * ⚠️ **`is_adset_budget_sharing_enabled` es obligatorio y es nuevo.** Sin él Meta rechaza con
 * «Debes indicar True o False… si no estás usando el presupuesto de la campaña» — otro campo que no
 * existía cuando estas campañas se armaron. Va en `false`: el reparto entre conjuntos es una
 * decisión de pauta, y el default silencioso de una herramienta no puede tomarla.
 *
 * ⛔ **Sólo ABO** (presupuesto por conjunto): la campaña nace sin `daily_budget`. Con CBO, el
 * presupuesto del conjunto no se puede poner, y toda la receta lo lleva adentro.
 */
export function recetaDeCampania({ objetivo, categorias } = {}) {
  if (!objetivoUsable(objetivo)) {
    return {
      ok: false, status: 409,
      error: `Meta ya no deja crear campañas con el objetivo «${objetivo || '(vacío)'}». Elegí como referencia un conjunto de una campaña con objetivo ${OBJETIVOS_VIVOS.join(', ')}.`,
    }
  }
  return {
    ok: true,
    cuerpo: {
      ...
      buying_type: 'AUCTION',
      special_ad_categories: JSON.stringify(Array.isArray(categorias) ? categorias : []),
