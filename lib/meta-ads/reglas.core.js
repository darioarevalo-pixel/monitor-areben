...
export const PRESETS = {
  ...
  'atribucion-tardia': {
    ...
    sub: 'pausar',
    /**
     * 🔴 **DATO, ⛔ no decisión** (5-sep-2026). Bruno tenía 19 pendientes abiertos y **cuatro eran
     * de este preset**, todos sobre avisos cuyo **conjunto padre estaba pausado**: reactivar el
     * aviso ⛔ no entrega nada. Y aunque el padre esté vivo, «Meta te atribuyó una compra dos días
     * después de que lo apagaste» ⛔ no es una decisión: es un dato que explica por qué el número
     * de ayer cambió. Sigue visible, ⛔ no en el contador.
     */
    clase: 'dato',
    proponeAccion: true,
    /**
  ...
  fatiga: {
    ...
    sub: 'pautar',
    /**
     * 🔴 **DATO, ⛔ no decisión** (5-sep-2026). El hallazgo real de ese día decía *«la misma gente lo
     * vio hasta 1,4 veces en un día (el máximo es 1,3)»*, y llevaba **seis días abierto**. Un 8%
     * por encima de un umbral que nadie calibró ⛔ no es una decisión: es la familia del *ruido con
     * cara de veredicto* que este módulo ya conoce. Sigue visible —el desgaste es real y hay que
     * verlo—, pero ⛔ no cuenta como algo pendiente de resolver.
     */
    clase: 'dato',
    proponeAccion: false,
  },
...
export function contarParaDecidir(hallazgos) {
  const cuentan = (hallazgos || []).filter(esParaDecidir)
  const total = cuentan.length
  const quemando = cuentan.filter(
...
/**
 * **Qué hallazgos abiertos hay que CERRAR porque la corrida de hoy ya ⛔ no los detecta.**
 *
 * # 🔴 Por qué existe (5-sep-2026)
 *
 * Bruno tenía **19 hallazgos abiertos** y ⛔ no accionaba ninguno. Cruzados uno por uno contra la
 * cuenta: **7 apuntaban a algo ya apagado**, 1 a un objeto que ⛔ ni aparecía en la ventana, y 1
 * contradecía a la tabla de la misma pantalla —decía «156% del techo, pausar» donde la fila decía
 * «Rinde, 58%»—. Ése era del **26-ago: diez días abierto**, con `veces: 1`.
 *
 * 📊 **Y el dato que ordena la solución**: las once reglas habían corrido el 4-sep a las 14:34Z, y
 * **13 de los 19 tenían `fecha` anterior a esa corrida**. O sea: *la regla ya había dejado de
 * detectarlos y nadie los cerró.* ⇒ ⛔ **no hace falta inventar un detector de obsolescencia** —los
 * detectores ya chequean `estaActivo()` y un objeto ausente ⛔ no produce grupo—: hacía falta
 * **cerrar**. Una sola regla cubre cuatro de las cinco formas del cementerio.
 *
 * 🔑 Es la MISMA lección que las celdas ya tenían resuelta con `configDeHoy()` y la clase
 * `apagada`: *proponer una acción ya hecha es el ruido que hace que se le deje de creer a la
 * pantalla*. El panel de hallazgos nunca la había recibido.
 *
 * # ⛔ Y por qué NO hay vencimiento por edad
 *
 * Con revalidación, un hallazgo de diez días es uno que la regla **confirma todas las mañanas y
 * nadie accionó**: vencerlo por viejo escondería justo el único que sí es real. Lo que ya existe es
 * lo contrario —`insistenciaDe()` lo envejece a la vista—, y recién ahora ese número dice la verdad.
 *
 * ⚠️ **Los tres candados están del lado del que llama** (`scripts/evaluar-reglas-meta.mjs`) y son
 * parte del contrato: ⛔ nunca con la regla `apagada`, ⛔ nunca en simulacro o calibración, ⛔ nunca
 * con `--hasta` en el pasado. Cerrar por «no lo detectó» una regla que ⛔ no pudo mirar es apagar un
 * aviso en silencio, que es el modo de falla más caro de este módulo.
 *
 * @param {Array} abiertos — los hallazgos en `nuevo` **de esta regla**.
 * @param {Array} detectados — lo que la corrida de hoy encontró (filas con `objeto_id`).
 * @returns {Array} los `id` a cerrar.
 */
export function aCerrarPorRevalidacion(abiertos, detectados) {
  const ids = (detectados || []).map((d) => d.objeto_id ?? d.objetoId)
  // 🔴 **Un id de Meta que llegue como NÚMERO ⇒ ⛔ no se cierra nada.** Son enteros de 17-18 dígitos
  // y `Number.MAX_SAFE_INTEGER` tiene 16: `120251374921030478` parseado como número vuelve
  // `120251374921030480`, y esa diferencia de dos dígitos hace que el hallazgo VIVO ⛔ no matchee y
  // se cierre. La precisión ya está perdida cuando llega acá, así que ⛔ no se puede reparar: lo
  // único correcto es no cerrar. Callar de más cuesta un hallazgo repetido; callar de menos apaga un
  // aviso real en silencio, que es lo que esta función ⛔ no puede hacer nunca.
  if (ids.some((id) => typeof id === 'number' && !Number.isSafeInteger(id))) return []
  const hoy = new Set(ids.map((id) => String(id)))
  return (abiertos || [])
    .filter((h) => !hoy.has(String(h.objeto_id ?? h.objetoId)))
    .map((h) => h.id)
    .filter((id) => id != null)
}

/**
 * **¿Este hallazgo es algo que hay que DECIDIR, o es un dato?**
 *
 * 🔴 Por qué existe (5-sep-2026). Bruno: *«son 14 pendientes que alargan la lista y que no estoy
 * ejecutando nada por ahí»*. Eran 19, y **cinco ⛔ no eran decisiones**: cuatro de
 * `atribucion-tardia` («Meta te atribuyó una compra después de que lo apagaste») y uno de `fatiga`
 * por una frecuencia 1,4 contra un máximo de 1,3. Ninguno tiene una mano del otro lado. Un contador
 * que los suma enseña a no mirar el contador.
 *
 * 🔑 **Es UNA función porque la leen TRES**: `contarParaDecidir` (el renglón de Rendimiento y la
 * pantalla Decidir), `armarMail` (el asunto de las 07:50) y `avisosDeHallazgo` (el badge del
 * sidebar). Copiada en tres, un preset nuevo entra en una sola y los otros dos cuentan otra cosa —
 * que es exactamente lo que `contarParaDecidir` existe para evitar entre la pantalla y el mail.
 *
 * ⚠️ Un preset **desconocido cuenta**. Si mañana entra uno sin `clase`, el default tiene que ser
 * «mostralo»: un aviso de más se ve y se corrige; uno de menos ⛔ no se entera nadie.
 */
export function esParaDecidir(h) {
  if (!h) return false
  const p = PRESETS[h.preset]
  return !p || p.clase !== 'dato'
}

/**
 * **Con qué números corta esta regla HOY**, para poder leerlo sin abrirla.
 *
 * # 🔴 Por qué (30-ago-2026)
 *
 * La fila colapsada decía nombre, Prendida/Apagada, el resumen del preset y «última corrida». Con
 * eso ⛔ no se puede contestar *«¿esta regla está bien calibrada?»* sin expandirla una por una — y son
 * once. Bruno: *«no se puede ver las configuraciones desde afuera; parecen unas listas que después
 * tengo que entrar a ver»*.
 *
 * 📊 **Y hay un número que lo hace peor: `meta_ads_umbral` tiene 0 filas.** Nadie movió un dial
 * nunca, así que las once cortan con el default **y la pantalla no lo dice en ninguna parte**.
 *
 * # Las cuatro procedencias, y ninguna se puede disfrazar de otra
 *
 *  - **`dial`** — alguien lo escribió. Es el único que se puede cambiar acá.
 *  - **`ficha`** — el techo de costo por compra, firmado en la ficha de rentabilidad de la marca.
 *  - **`derivado`** — sale de medir la pauta, y el número lo calcula el SERVIDOR sobre las filas de
 *    la foto. 🔑 La pantalla ⛔ **no lo inventa**: dice de dónde sale y ⛔ no cuál es. Escribir un
 *    número aproximado al lado de uno exacto es peor que no escribir ninguno.
 *  - **`falta`** — hay que decidirlo y nadie lo hizo ⇒ **la regla está apagada aunque figure
 *    prendida**, que es exactamente el defecto del freno de Stunned del 26-ago.
 */
export function cortesDe(preset, regla, umbralLinea, definicion, techo) {
  const p = preset || {}
  const def = definicion || {}
  const linea = umbralLinea || {}
  const propios = (regla && regla.parametros) || {}
  // El `requiereUno` entra igual: si su vara está sin cubrir, la regla no corta, y eso hay que verlo.
  const claves = [...(p.requiere || []), ...(p.requiereUno || [])]
  ...
  for (const u of claves) {
    ...
    vistas.add(u)
    const d = def[u] || {}
    const rotulo = d.rotulo || u
    if (d.desdeFicha) {
      const t = Number(techo)
      salida.push(Number.isFinite(t) && t > 0
        ? { clave: u, rotulo, valor: t, unidad: d.unidad || '', fuente: 'ficha' }
        : { clave: u, rotulo, valor: null, unidad: d.unidad || '', fuente: 'falta' })
      continue
    }
    const v = propios[u] ?? linea[u]
    if (v !== null && v !== undefined && Number.isFinite(Number(v))) {
      salida.push({ clave: u, rotulo, valor: Number(v), unidad: d.unidad || '', fuente: 'dial' })
      continue
    }
    salida.push({ clave: u, rotulo, valor: null, unidad: d.unidad || '', fuente: d.derivable ? 'derivado' : 'falta' })
  }
  return salida
}
