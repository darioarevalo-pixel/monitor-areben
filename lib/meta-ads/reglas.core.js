...
export const PRESETS = {
  ...
  'atribucion-tardia': {
    ...
    resumen: 'Algo que está pausado y que igual sumó compras en los últimos días.',
    porQue: 'Meta reatribuye conversiones durante ~72 h, así que un aviso puede haberse apagado por «no vende» y aparecer vendiendo dos días después. La foto diaria relee esa ventana justamente por eso, o sea que el dato ya está: lo único que faltaba era mirarlo.',
    nivel: 'aviso',
...
/**
 * 🔴 **El nivel del que salen los totales de una línea, y por qué no da igual cuál.**
 *
 ...
 * si tuvo entrega, y hay conjuntos que no cuelgan de ningún aviso vivo. El CPA sobreviviría al
 * triplicado —numerador y denominador escalan parejo— pero `gastoTotal`, que es lo que se le
 * muestra a una persona al lado del dial, no.
 */
export const NIVEL_TOTALES = 'campania'

/** Las filas de un nivel, descartando lo que no es de ese nivel. */
function soloNivel(filas, nivel) {
 ...
 * orden en que pasó) y `ultima`, que es la foto más reciente y de donde salen la configuración y el
 * estado: el nombre, la línea y el `estado_real` de hace cinco días no describen lo que hay hoy.
 */
export function agrupar(filas, nivel, fechas) {
  const dentro = new Set(fechas)
  const por = new Map()
  for (const f of Array.isArray(filas) ? filas : []) {
    if (!f || f.nivel !== nivel || !dentro.has(f.fecha)) continue
  ...
  return [...por.entries()].map(([objeto_id, fs]) => {
    ...
    return {
      ...
      filas: orden,
      ultima,
      nombre: ultima.nombre || objeto_id,
      linea: ultima.linea || null,
      cuenta_id: ultima.cuenta_id || null,
      dias: orden.length,
      spend,
      compras,
      revenue,
...
const DETECTORES = {
  'sin-avisos': (g) => {
    const real = String(g.ultima.estado_real || '')
    ...
    return {
      ...
      evidencia: { estado_real: real, estado: g.ultima.estado, diario_crudo: g.ultima.diario_crudo },
    }
  },

  'atribucion-tardia': (g) => {
    // Pausado HOY, con compras adentro de la ventana. Si estaba pausado y aun así sumó ventas, la
    // decisión de apagarlo se tomó con un número que después cambió.
    if (estaActivo(g.ultima)) return null
    if (g.compras <= 0) return null
    return {
      motivo: `Está pausado y aun así registró ${entero(g.compras)} ${g.compras === 1 ? 'compra' : 'compras'} por ${plata(g.revenue)} en los últimos ${g.dias} días. Meta se las atribuyó tarde.`,
      evidencia: { compras: g.compras, revenue: g.revenue, spend: g.spend, dias: g.dias, estado: g.ultima.estado },
  ...
  'ganador-escalar': (g, u) => {
    if (!estaActivo(g.ultima)) return null
    const diario = Number(g.ultima.diario_crudo) || 0
    ...
    const seguidos = diasSeguidosPorEncima(g.filas, u.roas_objetivo)
    const piden = Math.max(1, Math.floor(u.dias_seguidos || DIAS_SEGUIDOS_DEFECTO))
    if (seguidos < piden) return null
    ...
    const propuesto = Math.min(Math.round(diario * (1 + PASO_ESCALON)), Math.round(u.techo_diario_crudo))
    if (propuesto <= diario) return null
    return {
      motivo: `Lleva ${seguidos} días seguidos por encima del objetivo (${roasTxt(g.roas)} contra ${roasTxt(u.roas_objetivo)}) y todavía tiene margen de presupuesto.`,
      evidencia: { roas: g.roas, roas_objetivo: u.roas_objetivo, dias_seguidos: seguidos, diario_crudo: diario, techo_diario_crudo: u.techo_diario_crudo, propuesto_crudo: propuesto },
...
export function evaluarRegla(regla, { filas, umbralLinea, hasta }) {
  ...
  // Del más caro al más barato: el orden con el que se leen en el Panel es el orden en que
  // conviene mirarlos, y ordenarlo en la UI lo dejaría distinto entre pantallas.
  hallazgos.sort((a, b) => (Number(b.evidencia.spend) || 0) - (Number(a.evidencia.spend) || 0))
...
export function calibrar(regla, { filas, umbralLinea, hasta, dias = 90 }) {
  ...
  if (apagada) return { ok: true, apagada: true, faltan: apagada.faltan, detalle: apagada.detalle, dias, total: 0, objetos: 0, porFecha: [], ejemplos: [] }
