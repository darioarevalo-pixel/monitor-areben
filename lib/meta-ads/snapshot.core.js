...
export function estadoRealDe(estadoConjunto, avisos) {
  ...
  const vivos = avisos.filter((a) => {
    const s = String((a && (a.effective_status || a.status)) || '').toUpperCase()
    // Un aviso en revisión todavía puede terminar entregando: no cuenta como apagado.
