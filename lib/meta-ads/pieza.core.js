...
/**
 * ⭐ **La tabla de formatos ya no vive acá: vive en `lib/media.core.js`**, porque la galería de
 * Ingresos proyectados sube al Blob exactamente los mismos archivos y la alternativa era que se
 * escribiera su propia lista. Esa es la trampa que este bloque documentaba desde adentro de Meta
 * —llegó a haber tres listas sueltas y agregar un formato en una sola dejaba un archivo que la
 * pantalla acepta y el servidor rechaza—: entre secciones es el mismo error un piso más arriba.
 *
 * Los nombres de siempre siguen saliendo de acá para que nada de Meta se entere del movimiento.
 * `TIPOS_PIEZA` es hoy la lista entera; el día que Meta acepte algo que la galería no (o al revés),
 * se filtra en esta línea y no se parte la tabla.
 */
import {
  claseDeArchivo,
  CLASE_POR_EXTENSION,
...
import { validarDestino } from './destinos.core.js'

// Se re-exportan desde el binding importado (y no con `export … from`) porque `validarPiezas` usa
// `CLASE_POR_EXTENSION` acá adentro: un `export … from` re-exporta sin traer nada al scope.
export { CLASE_POR_EXTENSION, extensionDe, MIME_POR_EXTENSION }

/** Los MIME que el permiso de subida deja pasar (`allowedContentTypes`), sin repetidos. */
export const TIPOS_PIEZA = TIPOS_MEDIA

/** `'video' | 'imagen' | null`. `null` es «no sé qué es esto», que se rechaza antes de subir nada. */
export function claseDePieza(nombre) {
  return claseDeArchivo(nombre)
}

/** Con qué `contentType` sube este archivo, o `null` si la extensión no está en la tabla. */
export function mimeDePieza(nombre) {
  return mimeDeArchivo(nombre)
}
...
export function copyDeCreativo(cr) {
  ...
  if (!pageId) {
    return err(409, 'Ese aviso no dice de qué página sale, así que no se puede armar uno nuevo con su texto. Elegí otro de modelo.')
  }
...
/**
 * Los topes de largo de los textos que se escriben desde el monitor. **Son nuestros, no de Meta**:
 * Meta acepta más, pero un texto de 5.000 caracteres pegado por error sale al aire igual y nadie lo
 * lee. Están para frenar el pegado equivocado, no para editar el estilo.
 */
export const LARGO_TEXTOS = { mensaje: 2000, titulo: 255, descripcion: 255 }

const ROTULO_TEXTO = { mensaje: 'el texto', titulo: 'el título', descripcion: 'la descripción' }

/**
 * El copy del modelo con los **textos que alguien escribió** encima.
 *
 * 🔴 **Sólo se pisan `mensaje`, `titulo` y `descripcion`, y cualquier otra clave se ignora.**
 * `pageId` es lo que valida `puedeUsarLaPagina()` y `destino` es adónde va el click: si un campo
 * libre los pudiera cambiar, un editor de texto sería un editor de a dónde lleva el aviso. Salen
 * siempre del modelo, igual que `instagramId` y `cta`.
 *
 * - Clave **ausente** = se hereda la del modelo. Es lo que manda la pantalla cuando nadie tocó el
 *   campo, así que una tanda sin tocar sale idéntica a como salía antes de que existiera esto.
 * - Clave **presente** = pisa, recortada.
 * - ⛔ **Un `mensaje` vacío NO es «heredá el del modelo»: es un aviso sin texto, y se rechaza.**
 *   Decidido por Bruno el 12-sep-2026. El título y la descripción vacíos sí pasan: el aviso sale
 *   sin eso, y la pantalla lo avisa antes.
 *
 * `donde` completa el error («de la pieza «reel.mp4»») para que se sepa qué hay que corregir.
 */
export function textosDelCopy(copy, textos, donde) {
  if (!copy) return err(500, 'Falta el texto del aviso modelo.')
  const t = textos && typeof textos === 'object' ? textos : {}
  const sale = { ...copy }
  const cola = donde ? ` ${donde}` : ''
  for (const clave of Object.keys(LARGO_TEXTOS)) {
    if (!Object.prototype.hasOwnProperty.call(t, clave) || t[clave] === undefined) continue
    const valor = String(t[clave] === null ? '' : t[clave]).trim()
    if (valor.length > LARGO_TEXTOS[clave]) {
      return err(400, `${ROTULO_TEXTO[clave][0].toUpperCase()}${ROTULO_TEXTO[clave].slice(1)}${cola} pasa de ${LARGO_TEXTOS[clave]} caracteres.`)
    }
    if (clave === 'mensaje' && !valor) {
      return err(400, `Falta el texto del aviso${cola}: un aviso sin texto no se arma.`)
    }
    sale[clave] = valor || null
  }
  return { ok: true, copy: sale }
}

/**
 * Los botones que se pueden elegir. **Lista cerrada**: el tipo lo valida Meta, pero uno que no
 * corresponde a una tienda (`CALL_NOW`, `WHATSAPP_MESSAGE`) arma un aviso que Meta acepta y que no
 * lleva a comprar. `SHOP_NOW` y `LEARN_MORE` están medidos con `validate_only` el 15-sep-2026.
 */
export const CTAS = ['SHOP_NOW', 'LEARN_MORE', 'BUY_NOW', 'ORDER_NOW', 'SEE_MORE']

/** Tope de largo de los UTM. Nuestro, no de Meta: frena el pegado equivocado. */
export const LARGO_URL_TAGS = 500

/**
 * Las mejoras de Advantage+ que se apagan EXPLÍCITO en cada creativo nuevo.
 *
 * 🔑 **Medido el 15-sep-2026** (`scripts/ensayo-creativo-meta.mjs`, run 35011222530):
 * - Un creativo armado por este motor **ya nace con las 82 mejoras en `OPT_OUT`**, catálogo
 *   (`product_extensions`, `product_browsing`) y texto (`text_optimizations`) incluidos. El catálogo
 *   que «se prende solo» es de DUPLICAR en Ads Manager, no de este camino.
 * - Meta **acepta** escribir las funciones individuales en `OPT_OUT` y **rechaza** el campo viejo
 *   `standard_enhancements` (code 100 · subcode 3858504). Por eso acá nunca va ése.
 *
 * Se mandan igual, aunque hoy sean el default: el día que Meta cambie el default para cuentas
 * nuevas, una tanda de test con el catálogo prendido deja de medir la pieza. Van sólo las claves
 * que pasaron el spike.
 */
export const MEJORAS_APAGADAS = {
  creative_features_spec: Object.fromEntries(
    ['product_extensions', 'text_optimizations', 'inline_comment', 'enhance_cta', 'image_brightness_and_contrast', 'video_auto_crop']
      .map((k) => [k, { enroll_status: 'OPT_OUT' }]),
  ),
}

const idNumerico = (v) => /^\d{5,}$/.test(String(v || '').trim())

/**
 * UTM: pares `clave=valor` separados por `&`, sin `?` adelante y sin espacios. Las macros de Meta
 * (`{{campaign.name}}`) pasan. Vacío = sin UTM, que es válido.
 */
export function validarUrlTags(v) {
  const s = String(v === undefined || v === null ? '' : v).trim()
  if (!s) return { ok: true, urlTags: null }
  if (s.length > LARGO_URL_TAGS) return err(400, `Los UTM pasan de ${LARGO_URL_TAGS} caracteres.`)
  if (/^\?|\s/.test(s)) return err(400, 'Los UTM van sin «?» adelante y sin espacios: utm_source=meta&utm_medium=paid.')
  const pares = s.split('&')
  if (pares.some((p) => !/^[A-Za-z0-9_.-]+=[^=&]+$/.test(p))) {
    return err(400, 'Los UTM tienen que ser pares clave=valor separados por &.')
  }
  return { ok: true, urlTags: s }
}

/**
 * Lo que se elige del aviso además del texto: **destino, botón, página, Instagram y UTM**, encima
 * de un copy (el del modelo o uno vacío).
 *
 * 🔴 Esto reemplaza la regla vieja de «destino, página y botón salen SIEMPRE del modelo», y la
 * reemplaza con un guard por campo, **no con un campo libre**:
 * - `destino` pasa por `validarDestino()`: sólo la tienda de la línea. Medido el 15-sep: **Meta
 *   acepta un destino de cualquier dominio**, así que este guard es el único freno.
 * - `cta` sólo de `CTAS`.
 * - `pageId` / `instagramId` sólo como ids; que el token maneje la página lo sigue validando
 *   `puedeUsarLaPagina()` en el handler, sobre el copy YA ajustado.
 * - `urlTags` por `validarUrlTags()`.
 *
 * Clave ausente = queda lo de abajo. `donde` completa el error igual que en `textosDelCopy`.
 */
export function ajustesDelAviso(copy, ajustes, linea, donde) {
  const sale = { ...(copy || {}) }
  const a = ajustes && typeof ajustes === 'object' ? ajustes : {}
  const cola = donde ? ` ${donde}` : ''
  const hay = (k) => Object.prototype.hasOwnProperty.call(a, k) && a[k] !== undefined

  if (hay('destino')) {
    const v = validarDestino(a.destino, linea)
    if (!v.ok) return err(v.status, `${v.error}${cola ? ` (${cola.trim()})` : ''}`)
    sale.destino = v.destino
  }
  if (hay('cta')) {
    const c = String(a.cta || '').trim().toUpperCase()
    if (!CTAS.includes(c)) return err(400, `El botón «${a.cta}»${cola} no está entre los que se pueden elegir: ${CTAS.join(', ')}.`)
    sale.cta = c
  }
  if (hay('pageId')) {
    if (!idNumerico(a.pageId)) return err(400, `La página${cola} no es un id de Meta.`)
    sale.pageId = String(a.pageId).trim()
  }
  if (hay('instagramId')) {
    if (a.instagramId === null || a.instagramId === '') sale.instagramId = null
    else if (!idNumerico(a.instagramId)) return err(400, `La cuenta de Instagram${cola} no es un id de Meta.`)
    else sale.instagramId = String(a.instagramId).trim()
  }
  if (hay('urlTags')) {
    const u = validarUrlTags(a.urlTags)
    if (!u.ok) return u
    sale.urlTags = u.urlTags
  }
  return { ok: true, copy: sale }
}

/**
 * El copy **sin aviso modelo**: todo sale del formulario. Lo que en `copyDeCreativo` son los tres
 * rechazos del modelo, acá son los tres obligatorios: página, destino y texto.
 */
export function copyDelFormulario(formulario, linea) {
  const f = formulario && typeof formulario === 'object' ? formulario : {}
  if (!idNumerico(f.pageId)) return err(400, 'Falta elegir la página desde la que sale el aviso.')
  if (!f.destino) return err(400, 'Falta el destino: un aviso sin destino no lleva a ningún lado.')
  const base = { pageId: null, instagramId: null, mensaje: null, titulo: null, descripcion: null, destino: null, cta: 'SHOP_NOW', urlTags: null }
  const aj = ajustesDelAviso(base, {
    pageId: f.pageId, destino: f.destino,
    ...(f.instagramId !== undefined ? { instagramId: f.instagramId } : {}),
    ...(f.cta !== undefined ? { cta: f.cta } : {}),
    ...(f.urlTags !== undefined ? { urlTags: f.urlTags } : {}),
  }, linea)
  if (!aj.ok) return aj
  const tx = textosDelCopy(aj.copy, { mensaje: f.mensaje ?? '', titulo: f.titulo, descripcion: f.descripcion })
  if (!tx.ok) return tx
  return { ok: true, copy: tx.copy }
}

/** Lo que llegó como ajustes de una pieza o de la tanda, reducido a las claves que se pueden pisar. */
export function soloAjustes(t) {
  if (!t || typeof t !== 'object') return null
  const sale = {}
  for (const clave of ['destino', 'cta', 'pageId', 'instagramId', 'urlTags']) {
    if (t[clave] !== undefined && (typeof t[clave] === 'string' || t[clave] === null)) sale[clave] = t[clave]
  }
  return Object.keys(sale).length ? sale : null
}

/** Lo que llegó como textos de una pieza o de la tanda, reducido a las tres claves que se pisan. */
export function soloTextos(t) {
  if (!t || typeof t !== 'object') return null
  const sale = {}
  for (const clave of Object.keys(LARGO_TEXTOS)) {
    if (typeof t[clave] === 'string') sale[clave] = t[clave]
  }
  return Object.keys(sale).length ? sale : null
}
...
export function cuerpoDeCreativo(copy, pieza) {
  const c = copy || {}
  const p = pieza || {}
  if (!c.pageId) return err(500, 'Falta la página del texto: no se puede armar el creativo.')
  ...
  return {
    ...
    cuerpo: {
      ...
      // Las mejoras apagadas explícito, con la forma que Meta aceptó (ver `MEJORAS_APAGADAS`).
      degrees_of_freedom_spec: JSON.stringify(MEJORAS_APAGADAS),
      ...(c.urlTags ? { url_tags: c.urlTags } : {}),
    },
  }
}
...
export function validarPiezas(piezas) {
  ...
  for (const p of ps) {
    ...
    // El texto propio de la pieza, si lo tiene. Viaja reducido a las tres claves que se pueden pisar:
    // lo demás que venga en el cuerpo no llega ni al plan.
    const textos = soloTextos(p.textos)
    // Y sus ajustes propios (hoy, sobre todo el destino: un video de UNA funda puede ir a su
    // producto). Se validan de verdad al armar el plan, con la línea, en `ajustesDelAviso`.
    const ajustes = soloAjustes(p.ajustes)
    salida.push({ nombre, url, clase, ...(textos ? { textos } : {}), ...(ajustes ? { ajustes } : {}) })
  }
