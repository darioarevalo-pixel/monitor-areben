...
/**
 * Seguidores nuevos. Mismo criterio y mismo cuidado que RE_PERFIL: Meta los nombra distinto según
 * la superficie (`follow` de Instagram, `page_like` de una página de Facebook), así que se matchea
 * por patrón.
 *
 * 🔴 **`like` PELADO ⛔ NO es un seguidor, y contarlo MINTIÓ EN PANTALLA.** La versión anterior
 * cerraba con `(^|\.)like$`, que matchea el `action_type` `like` a secas. En una campaña de
 * Instagram eso son **«me gusta» de publicación**, ⛔ no seguimientos. Medido el 18-sep-2026 sobre
 * `TRAFICO - STUNNED - PERFIL IG - 12/9`: la pantalla decía **14 seguidores a $1.773 cada uno**,
 * cuando @stunned.co ganó **+56 seguidores reales** en esa ventana (serie diaria de MAKETA) a
 * **$443**. El cartel estaba 4× arriba, y encima sobre una métrica que ⛔ ni siquiera era la que
 * nombraba.
 *
 * ⚠️ **El dato bueno ⛔ no está en `actions`.** Esta cuenta ⛔ no devuelve ningún `action_type` de
 * seguimiento ni de visita al perfil —se miraron las 32 que sí trae—; Meta los muestra en Ads
 * Manager bajo «Conversiones». ⇒ acá el valor honesto es **0**, que `Rendimiento.tsx` ya sabe
 * dibujar como «Sin visitas ni seguidores», con la lista de acciones reales en el `title`. El costo
 * por seguidor de verdad se mide contra las estadísticas de Instagram, como dice `PENDIENTES.md`.
 *
 * `[._]` y ⛔ no sólo `\.`: así entran `ig_follow` y `onsite_conversion.follow` por igual.
 */
export const RE_SEGUIDOR = /(^|[._])follow|page_like/i
...
export function metricasDe(row) {
  return {
    ...
    // 🔴 `clicks` son TODOS los clicks (me gusta, comentario, compartir, agrandar la foto, "ver
    // más", ir al perfil); `inline_link_clicks` son sólo los que van a la web. No son
    // intercambiables y restarle `lpv` a `clicks` para medir el escalón click→landing da un
    // número inflado y creíble — pasó el 27-ago-2026. Ver el comentario en la migración.
