...
import { ajustesDelAviso, textosDelCopy, TOPE_PIEZAS } from './pieza.core.js'
...
export function armarPlanDuplicar(entrada, marcador) {
  ...
  /**
   * 🔑 **Un conjunto por aviso**: en vez de una copia con todos los avisos adentro, reparte los
   * avisos del original en un conjunto cada uno, con la misma receta.
   *
   * Es el mismo criterio que ordena `armarPlanPiezas` y por el mismo motivo: **Meta reparte el
   * presupuesto de un conjunto entre sus avisos según lo que va aprendiendo**, así que dos piezas
   * adentro del mismo conjunto no compiten parejo — una se lleva casi todo a los dos días y la otra
   * no perdió, no jugó. La diferencia con `armarPlanPiezas` es de dónde sale la pieza: allá se sube
   * un archivo, acá se reusa un `creative_id` que YA existe en la cuenta. Sin esto, una pieza que
   * quedó sin entrega adentro de un conjunto sólo se podía volver a probar subiendo el video de
   * nuevo, que le hace perder el social proof.
   */
  const unoPorAviso = Boolean(e.unoPorAviso)
  if (unoPorAviso && nivel !== 'conjunto') {
    return err(400, 'Repartir en un conjunto por aviso sólo se puede sobre un conjunto.')
  }
  // ⛔ No se combina con `copias`: repartir YA es hacer varios conjuntos, y pedir «3 copias de un
  // reparto de 4 avisos» son 12 conjuntos que nadie quiso.
  if (unoPorAviso && copias !== 1) {
    return err(400, 'Repartir ya crea un conjunto por aviso, así que no se pide además una cantidad de copias.')
  }
  const censo = e.censo || {}
  const pasos = []
  ...
  if (unoPorAviso) {
    ...
    const avisos = censo.avisos || []
    // El 400 dice qué falta y no «no hay nada que copiar»: acá el pedido tiene sentido y lo que
    // falta es la selección, que es lo que quien lo pidió puede arreglar.
    if (!avisos.length) return err(400, 'No hay ningún aviso para repartir.')

    for (const a of avisos) {
      // 🔑 El nombre del conjunto lleva el del aviso, y no es cosmético: cuando después se miren los
      // números en Ads Manager, **es lo único que dice qué pieza es cuál**. Mismo criterio que
      // `armarPlanPiezas`.
      const suyo = `${nombreBase} · ${String(a.nombre || 'aviso')}`.slice(0, LARGO_NOMBRE)
      const nConj = push('crear-conjunto', `Crear el conjunto de «${a.nombre}»`, {
        cuentaId: String(e.cuentaId),
        campaignId: String(e.campaignId || ''),
        nombreBase: suyo,
        cuerpo: e.receta.cuerpo,
      })
      push('crear-aviso', `Crear el aviso «${a.nombre}»`, {
        cuentaId: String(e.cuentaId),
        adsetId: `{{${nConj}}}`,
        creativeId: String(a.creativeId || ''),
        nombreBase: String(a.nombre || 'aviso'),
      })
      // El nombre definitivo va SIEMPRE —no sólo cuando lo pidieron, como en la copia normal—
      // porque acá el nombre ES el resultado: sin él quedan N conjuntos con la marca del plan y sin
      // manera de saber cuál tiene cuál pieza.
      push('nombre', `Ponerle el nombre definitivo al conjunto de «${a.nombre}»`, {
        objetoId: `{{${nConj}}}`,
        name: suyo,
      })
    }
    return { ok: true, pasos, variante: 'uno-por-aviso' }
  }
 ...
 * - `piezas`: `[{ nombre, url, clase, textos? }]` ya validadas por `validarPiezas()`
 * - `copy`: el de `copyDeCreativo()`, leído del aviso modelo
 * - `textos`: los de la tanda, escritos desde el monitor (opcional)
 *
 * # El copy es POR PIEZA
 *
 * Cada creativo sale con `modelo ← textos de la tanda ← textos de la pieza`, en ese orden. Decidido
 * por Bruno el 12-sep-2026, con la misma razón del 1-sep: el copy es del AVISO, y atar todas las
 * piezas a un texto que no le queda bien a ninguna las perjudica a todas. ⚠️ Lo que se resigna,
 * escrito para que no se lea como descuido: si una pieza con texto propio gana, no se puede separar
 * cuánto fue la pieza y cuánto el texto. La página, el destino y el botón no se tocan nunca: ver
 * `textosDelCopy()`.
...
export function armarPlanPiezas(entrada, marcador) {
  ...
  // Los textos se resuelven TODOS antes de armar un solo paso: un texto vacío en la pieza 5 no puede
  // dejar un plan con cuatro piezas adentro.
  const deTanda = textosDelCopy(e.copy, e.textos, 'de la tanda')
  if (!deTanda.ok) return deTanda
  const copyDe = new Map()
  for (const p of piezas) {
    // Primero lo que se elige de la pieza (un destino propio, por ejemplo), con la línea de la campaña:
    // el guard del destino corre acá también, no sólo en el handler. Después su texto.
    const aj = ajustesDelAviso(deTanda.copy, p.ajustes, e.linea, `de «${p.nombre}»`)
    if (!aj.ok) return aj
    const r = textosDelCopy(aj.copy, p.textos, `de «${p.nombre}»`)
    if (!r.ok) return r
    copyDe.set(p, r.copy)
  }
  ...
  for (const p of piezas) {
    ...
    if (p.clase === 'video') {
      ...
      nCreativo = push('crear-creativo', `Armar el creativo de «${p.nombre}»`, {
        ...
        copy: copyDe.get(p), nombreBase: base(p),
      })
    } else {
      nCreativo = push('crear-creativo', `Armar el creativo de «${p.nombre}»`, {
