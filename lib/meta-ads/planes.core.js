 ...
 * - `censo`: `{ conjuntos: [{id, nombre, avisos:[{id,nombre,creativeId}]}] }` para una campaña,
 *   `{ avisos: [{id,nombre,creativeId}] }` para un conjunto.
 */
export function armarPlanDuplicar(entrada, marcador) {
  ...
  for (let i = 1; i <= copias; i++) {
    ...
    if (nivel === 'conjunto') {
      const nCopia = push('copiar-conjunto', `Copiar el conjunto${cual}`, {
      ...
      if (e.presupuestoCrudo) {
        push('presupuesto', `Ponerle el presupuesto diario a la copia${cual}`, {
          objetoId: `{{${nCopia}}}`,
          daily_budget: String(Math.round(Number(e.presupuestoCrudo))),
        })
      }
      if (e.nombre) {
    ...
    for (const c of censo.conjuntos || []) {
      const nConj = push('copiar-conjunto', `Copiar el conjunto «${c.nombre}»${cual}`, {
        objetoId: String(c.id),
        campaignId: `{{${nCamp}}}`,
      })
      ...
      if (e.presupuestoCrudo && (censo.conjuntos || []).length === 1) {
        push('presupuesto', `Ponerle el presupuesto diario al conjunto${cual}`, {
          objetoId: `{{${nConj}}}`,
          daily_budget: String(Math.round(Number(e.presupuestoCrudo))),
        })
      }
