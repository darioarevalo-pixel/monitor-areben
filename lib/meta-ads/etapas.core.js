 ...
 *
 * La sección Meta Ads muestra plata y ROAS, pero ⛔ no muestra **a quién le estás hablando**. Esto
 * clasifica cada campaña en una etapa para que el hueco se vea.
 *
 * ## ⛔ CORREGIDO el 30-ago-2026: la premisa que estaba escrita acá era FALSA, y era al revés
 *
 * Decía: *«Toda la pauta de BDI y Zattia es de la primera etapa (tráfico y reconocimiento). Nadie
 * arma pauta de las otras dos»*. 📊 Medido sobre la foto de agosto (10 campañas, 1→29 de agosto):
 *
 * | etapa | campañas AL AIRE | gasto del mes |
 * |---|---|---|
 * | **BOFU** «que compren» | **5** | **$1.472.073 — el 84%** |
 * | **TOFU** «que te conozcan» | 3 | $284.509 |
 * | **MOFU** «que te consideren» | **0** | **$0** |
 *
 * ⇒ 🔴 **el agujero ⛔ no es el de arriba del embudo: es el del MEDIO, y está en CERO.** La premisa
 * vieja mandaba a mirar justo donde sí hay pauta.
 *
 * 🔑 **Y la conducta ya estaba bien**: `veredictoDe()` prioriza la segunda etapa vacía sobre todo lo
 * demás, así que la pantalla venía señalando MOFU aunque el comentario dijera otra cosa. Es
 * exactamente por qué una premisa **se vuelve test y ⛔ no se deja escrita**: la frase envejeció tres
 * meses sin que nada se pusiera rojo, y quien la leyera antes de tocar el módulo habría "arreglado"
 * la prioridad para que apuntara a TOFU. El test que la clava está en `tests/meta-ads-etapas.test.ts`
 * («la forma real de la cuenta»).
 *
...
/**
 * **El censo de campañas armado desde la FOTO** — para cuando Graph ⛔ no contesta.
 *
 * # Por qué existe
 *
 * 🔴 `?recurso=etapas` está **debajo** del guard de `META_ADS_TOKEN` en `api/meta-ads.js`, a
 * diferencia de la zona de Rendimiento, las automatizaciones y los informes, que están **arriba a
 * propósito**: el día que el token se venza, la pregunta *«¿a quién le estoy hablando?»* sigue
 * siendo la misma, y el Embudo se moría con un 500. Y el dato ya estaba: `meta_ads_snapshot_dia`
 * guarda `objetivo` a nivel campaña.
 *
 * # 🔴 Las tres cosas que la foto ⛔ NO puede contestar, y que por eso VIAJAN EN LA RESPUESTA
 *
 * 1. **`objetivo` empezó a guardarse el 8-ago-2026.** 📊 De las 552 filas de campaña de toda la
 *    foto, **423 (el 77%) ⛔ no lo tienen**, y son las de mayo a agosto. ⇒ una campaña sin objetivo
 *    en NINGUNA de sus filas **⛔ no entra al censo**: contarla como `sin-clasificar` afirmaría
 *    «nadie le puso etapa», cuando lo que pasa es que **la foto ⛔ no lo guardaba**. Sale aparte, en
 *    `sinObjetivo`, y la pantalla lo dice. 📌 Es el cero que afirma, otra vez.
 * 2. **La foto sólo tiene lo que APARECIÓ EN INSIGHTS.** Una campaña `ACTIVE` que ⛔ nunca entregó
 *    ⛔ no tiene fila ⇒ el renglón «activas sin entrega» sale **incompleto**, y ése es justo el que
 *    destapa los conjuntos rotos. Va dicho: `completo: false`.
 * 3. **Sólo días CERRADOS**, y el último lo decide `ultimoDiaCerrado()` sobre estas mismas filas.
 *
 * ⚠️ **Es un RESPALDO, ⛔ no un reemplazo.** Con el token vivo manda Graph, que trae también las
 * campañas de $0 y el presupuesto de todas. Devolver esto callado, con cara de censo completo,
 * sería peor que el 500 que reemplaza.
 *
 * @param filas  filas de la foto a nivel `campania`, ya recortadas a la ventana
 * @returns `{ campanias, cuentas, sinObjetivo, completo }`
 */
export function censoDeLaFoto(filas) {
  const porId = new Map()
  const cuentas = new Map()
  for (const f of Array.isArray(filas) ? filas : []) {
    if (!f || f.nivel !== 'campania' || !f.objeto_id) continue
    const k = String(f.objeto_id)
    const a = porId.get(k) || {
      id: k, nombre: '(sin nombre)', cuentaId: '', objetivo: null, estado: null,
      diarioCrudo: 0, totalCrudo: 0,
      spend: 0, impressions: 0, clicks: 0, purchases: 0, revenue: 0,
      _ultima: '',
    }
    a.spend += n(f.spend)
    a.impressions += n(f.impresiones)
    a.clicks += n(f.clicks)
    a.purchases += n(f.compras)
    a.revenue += n(f.revenue)
    // 🔑 **El nombre, el estado y el presupuesto salen del día MÁS NUEVO**, igual que en
    // `celdasDeLaFoto()`: la foto los guarda al momento de sacarla, así que una campaña renombrada
    // tiene dos nombres en su serie y el primero ya no existe en Ads Manager.
    const fecha = String(f.fecha || '').slice(0, 10)
    if (fecha >= a._ultima) {
      a._ultima = fecha
      a.nombre = f.nombre || a.nombre
      a.cuentaId = f.cuenta_id ? String(f.cuenta_id) : a.cuentaId
      // ⚠️ `estado_efectivo` y ⛔ no `estado`: es el que dice si está al aire de VERDAD — `estado`
      // contesta ACTIVE aunque la cuenta esté frenada. Es la misma elección que hace el handler.
      a.estado = f.estado_efectivo || f.estado || a.estado
      a.diarioCrudo = f.diario_crudo == null ? a.diarioCrudo : n(f.diario_crudo)
    }
    // 🔴 El objetivo ⛔ NO sale del último día: sale del último día que lo TENGA. Una campaña que
    // dejó de entregar vuelve a escribir filas con `objetivo: null` (medido: la de Stunned, el
    // 13-ago), y quedarse con el último la mandaría a `sinObjetivo` teniendo el dato dos filas atrás.
    if (f.objetivo) a.objetivo = f.objetivo
    porId.set(k, a)
    if (f.cuenta_id && !cuentas.has(String(f.cuenta_id))) {
      cuentas.set(String(f.cuenta_id), { id: String(f.cuenta_id), nombre: `Cuenta ${f.cuenta_id}`, moneda: f.moneda || '' })
    }
  }
  ...
  for (const c of porId.values()) {
    const { _ultima, ...limpia } = c
    if (!limpia.objetivo) { sinObjetivo.push(limpia); continue }
    campanias.push({ ...limpia, etapaAuto: etapaDeObjetivo(limpia.objetivo) })
  }
  campanias.sort((a, b) => b.spend - a.spend)
  sinObjetivo.sort((a, b) => b.spend - a.spend)
  return { campanias, cuentas: [...cuentas.values()], sinObjetivo, completo: false }
}

/** Un número de la foto, o 0. Las columnas vienen como string desde PostgREST más de una vez. */
function n(v) {
  const x = Number(v)
