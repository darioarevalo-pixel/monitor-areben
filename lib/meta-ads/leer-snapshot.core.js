...
import { calcularRentabilidad, normalizar } from './rentabilidad.core.js'
...
export const TABLA_RENTABILIDAD = 'meta_ads_rentabilidad'

/**
 * **El techo de costo por compra de cada línea**, calculado desde la ficha guardada.
 *
 * Vive acá por lo mismo que `leerUmbrales()`: lo piden la zona de Rendimiento, las automatizaciones,
 * el calibrador y el guardarraíl de los escalones. La ficha guarda **supuestos**, no el techo, así
 * que cada consumidor que la lea tiene que volver a calcularlo — y cuatro copias de un cálculo
 * económico son cuatro techos que un día no coinciden.
 *
 * 🔴 **Devuelve el techo de GANANCIA (`costoMax`), ⛔ nunca el de caja.** El de caja incluye el
 * recupero del saldo de IVA, que es plata real pero que la venta sólo libera: no se repite, no
 * depende de la calidad de la venta y no la genera la pauta. Una regla calibrada sobre él arma una
 * estructura encima de un stock finito. El de caja viaja al lado, para mostrarlo, y separado.
 *
 * ⚠️ Una ficha ilegible es una línea **sin techo**, no un error: se saltea. Y una línea sin fila no
 * entra al mapa ⇒ quien la consulte recibe `undefined`, que es lo que deja la regla apagada
 * diciendo que falta la ficha. ⛔ Nunca un default: un techo inventado se lee igual que uno medido.
 */
export async function leerTechos(sb, lineas = null) {
  let q = sb.from(TABLA_RENTABILIDAD).select('linea, supuestos, updated_at')
  // Mismo criterio que `leerSnapshot` y `leerDecisiones`: con `[]` no filtra nada.
  if (Array.isArray(lineas) && lineas.length) q = q.in('linea', lineas)
  ...
  for (const f of data || []) {
    try {
      /**
       * 🔴 **Una fila sin `precio` NO es una ficha de esa marca: es la economía de BDI con otro
       * nombre.** `normalizar()` arranca de `DEFAULTS` —que son las fundas de BDI— y le pisa
       * encima lo que la fila traiga, así que unos supuestos vacíos, o ilegibles, devuelven el
       * techo de BDI para cualquier línea. Para la PANTALLA eso está bien y está dicho («esto es
       * el arranque, cargá tu economía»); para una regla que propone APAGAR es un techo inventado
       * decidiendo plata, y encima el de otro negocio. El precio es el mínimo que hace propia a
       * una ficha.
       */
      const sup = f.supuestos
      if (!sup || typeof sup !== 'object' || !(Number(sup.precio) > 0)) continue
      const r = calcularRentabilidad(normalizar(sup))
      if (!(r.costoMax > 0)) continue
      mapa.set(f.linea, {
        techo: r.costoMax,
        techoCaja: r.costoMaxCaja !== r.costoMax ? r.costoMaxCaja : null,
        cargadaEl: f.updated_at || null,
        ticket: r.ticket,
      })
    } catch { /* una ficha ilegible es una línea sin techo, no un 500 */ }
  }
  return { mapa }
}

/** El número solo, para quien no necesita la ficha entera. `0` si esa línea no tiene. */
export function techoDe(mapa, linea) {
  const f = mapa && mapa.get ? mapa.get(linea) : null
  return f && f.techo > 0 ? f.techo : 0
}
...
export async function contextoDeFoto(sb, linea, hasta, ventana) {
  ...
  const [u, snap, t] = await Promise.all([
    leerUmbrales(sb),
  ...
  // 🔴 El techo viaja SIEMPRE con los umbrales, por el mismo motivo por el que éstos se releen en
  // cada paso: bajar el techo de una marca tiene que frenar lo que ya está en curso. Un error de
  // lectura acá deja la línea sin techo, que es el modo seguro —el guardarraíl no deja pasar nada—,
  // ⛔ nunca «seguí con el default».
  if (t.error) return { error: `No se pudo leer la ficha de rentabilidad: ${t.error}` }
  return { umbralLinea: u.mapa.get(linea) || null, filas: snap.filas || [], techo: techoDe(t.mapa, linea) }
}
