...
import { gravedadDeHallazgo, ORDEN_GRAVEDAD } from './reglas.core.js'
...
export function asuntoDe(hallazgos) {
  ...
  const cuantas = n === 1 ? '1 cosa para decidir' : `${n} cosas para decidir`
  if (!queman) return `Pauta · ${cuantas}`
  return `Pauta · ${cuantas}, ${queman === 1 ? '1 quemando plata' : `${queman} quemando plata`}`
}
...
export function armarMail(hallazgos, hoy) {
  const vivos = (hallazgos || []).filter(Boolean)
