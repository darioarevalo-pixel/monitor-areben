 ...
 *
 * ⚠️ `timeoutMs` es un parámetro y no una constante porque **el motor de planes encadena POSTs
 * adentro de un mismo request** (`TIMEOUT_PASO_MS`, 5 s): ahí un paso colgado no se puede comer la
 * función entera, que es el tiempo de los pasos que faltan. El default deja igual a quien lo llama
 * con dos argumentos, que es todo el resto del repo.
 */
export async function graphPost(path, params, timeoutMs = TIMEOUT_MS) {
