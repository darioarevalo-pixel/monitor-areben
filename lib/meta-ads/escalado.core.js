...
import {
  CON_AIRE, DIAS_SEGUIDOS_DEFECTO, PASO_ESCALON, VENTANA_DIAS,
  agrupar, faltanUmbrales, fraseDeRacha, hayRacha, motivoApagada, proximoDiario, ventanaDe,
} from './reglas.core.js'
...
export function decidirEscalon({ objetoId, nivel = 'conjunto', diarioCrudo, filas, umbrales, hasta, moneda = 'ARS', ventana = VENTANA_DIAS } = {}) {
  ...
  // 5. La racha, contra la foto de hoy. **La misma cuenta que hace el detector que lo propuso**, y
  //    por eso sale de `hayRacha()` y no de una condición escrita acá. Desde el 26-ago-2026 la vara
  //    es el COSTO si la marca tiene ficha de rentabilidad, y el ROAS sólo si no la tiene.
  const racha = hayRacha(g.filas, u)
  if (!racha.ok) {
    // Las dos mitades de la vara del costo frenan por motivos distintos, y el que lee esto tres días
    // después necesita saber cuál de las dos fue: «viene caro» se arregla apagando, «viene bien pero
    // sin aire» se arregla esperando.
    const motivo = racha.vara === 'costo' && racha.conAire === false
      ? `${racha.cpa === null ? 'No compró nada en la ventana' : `Compra a ${plata(racha.cpa)} contra un techo de ${plata(racha.objetivo)} (${Math.round((racha.cpa / racha.objetivo) * 100)}%)`}, y para subirle plata pide estar debajo del ${CON_AIRE}% del techo: un escalón casi siempre encarece antes de asentarse. No se le sube.`
      : racha.vara === 'costo'
        ? `Pedía ${racha.piden} días confirmados comprando debajo del techo (${plata(racha.objetivo)}) y lleva ${racha.seguidos === 0 ? 'ninguno' : racha.seguidos}, en los últimos ${g.dias} días. No se le sube.`
        : `Pedía ${racha.piden} días seguidos por encima de ${roasTxt(u.roas_objetivo)} y lleva ${racha.seguidos === 0 ? 'ninguno' : racha.seguidos}: viene en ${roasTxt(g.roas)} en los últimos ${g.dias} días. No se le sube.`
    return {
      seguir: false,
      motivo,
      evidencia: {
        vara: racha.vara, cpa: racha.cpa, cpa_maximo: u.cpa_maximo, con_aire: racha.conAire,
        roas: g.roas, roas_objetivo: u.roas_objetivo, dias_seguidos: racha.seguidos, piden: racha.piden,
  ...
  return {
    ...
    desdeCrudo: desde,
    aCrudo,
    motivo: `${fraseDeRacha(racha, g)}: de ${plata(aMonto(desde, moneda))} a ${plata(aMonto(aCrudo, moneda))} por día.`.replace(/^./, (c) => c.toUpperCase()),
    evidencia: {
      vara: racha.vara, cpa: racha.cpa, cpa_maximo: u.cpa_maximo,
