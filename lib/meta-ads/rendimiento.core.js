...
import { comprasParaAfirmar, cruzarConLaCaja, marginalEntreVentanas, veredictoDeCosto } from './parte.core.js'
...
/**
 * Los escalones a los que se estira la ventana de JUICIO cuando la muestra no alcanza para decidir.
 * Son los mismos días que ya ofrece la barra, así que ⛔ no hay una ventana que la pantalla no sepa
 * nombrar. Ver `armarZona`.
 */
export const ESCALONES_JUICIO = [7, 14, 30]
/** Cuánto sube un escalón. 20% es el paso que Meta recomienda para no reiniciar el aprendizaje. */
export const PASO_ESCALON = 0.2

/**
 * **El par ESTADO / MANO de cada clase de veredicto, y su color.** Tres mapas y ⛔ ningún `if` en
 * la pantalla.
 *
 * # 🔴 Por qué son datos y por qué viven acá (5-sep-2026)
 *
 * Bruno: *«el no hay que hacer nada y toda la tipografía no está en infinitivo, osea no está bien
 * armado»*. Tenía razón y el arreglo ⛔ no era traducir diez títulos: era **separar dos cosas que
 * estaban en el mismo string**. Un pill decía `Compra arriba del techo` —un ESTADO, en presente— y
 * lo que hace falta leer de un vistazo es **la MANO**: `Pausar`. `VOCABULARIO.md` §3 pide infinitivo
 * para los títulos de acción, y un estado en pretérito de diez palabras adentro de un pill ⛔ no es
 * ninguna de las dos cosas.
 *
 * ⇒ **el estado va como sustantivo corto** (un badge al lado del nombre) y **la mano como
 * infinitivo** (el pill). Y `accion` —que existía en el tipo desde siempre y **⛔ no se dibujaba en
 * ningún lado**— pasa a ser lo que manda.
 *
 * 🔑 **`mano: null` es una decisión de diseño, ⛔ no un hueco.** Con el reparto real de la cuenta
 * (2 pautas caras, 2 que rinden, el resto sin prueba) un pill por fila sería un muro gris que ⛔ no
 * señala nada. El pill se dibuja **sólo donde hay algo que hacer**; en el resto el color se ahorra
 * y lo que ordena es el orden de las filas.
 *
 * Están acá y ⛔ no en el JSX para que una clase nueva entre con **tres renglones de datos y cero
 * cambios de pantalla** — y para que `tests/meta-veredicto-mano.test.ts` pueda exigir que ninguna
 * clase quede sin sus tres etiquetas.
 */
export const ESTADO_DE_CLASE = {
  apagada: 'Apagada',
  rota: 'Sin avisos',
  quieta: 'Sin entrega',
  'sin-techo': 'Sin techo',
...
export const MANO_DE_ACCION = {
  pausar: 'Pausar',
  escalar: 'Escalar',
  revisar: 'Revisar',
  'cargar-techo': 'Cargar el techo',
}

/** El tono de cada clase. `warning` en `sin-prueba` a propósito: `danger` afirmaría que se quema. */
export const TONO_DE_CLASE = {
  apagada: 'neutral',
  rota: 'danger',
  quieta: 'warning',
  'sin-techo': 'warning',
...
/**
 * El orden de la tabla: **primero lo que pide una mano, y lo apagado al fondo.**
 *
 ...
 *
 * # 🔴 Y por qué el desempate ya ⛔ no es el gasto (5-sep-2026)
 *
 * Con el piso de evidencia, la mayoría de las filas cae en `sin-prueba` —arriba del techo pero
 * dentro del ruido— y ninguna dibuja pill. Si ese montón se ordenara por **gasto**, la que hay que
 * mirar primero quedaría enterrada: lo que importa ⛔ no es cuánto gastó, es **cuánto pagó DE MÁS**.
 * Medido el 5-sep en BDI: `TEST FUNDAS` gastó $32.146 y pagó $9.927 de más, mientras `TEST BROAD
 * BDI` gastó $49.770 y pagó $5.634 de más. Por gasto sale al revés de como hay que mirarlas.
 *
 * ⇒ **grupo por clase, y adentro por `excedente`**, cayendo al gasto cuando no hay excedente (sin
 * techo, sin compras). Es el orden que contesta *«¿cuál miro primero?»*, que es lo único que le
 * queda por decir a una fila que ⛔ no propone nada.
 *
...
export function ordenarCeldas(celdas) {
  const pri = (c) => {
    const k = c.veredicto && c.veredicto.clase
    return k in PRIORIDAD_CLASE ? PRIORIDAD_CLASE[k] : PRIORIDAD_CLASE.ok
  }
  return [...(celdas || [])].sort((a, b) => {
    const d = pri(a) - pri(b)
    if (d) return d
    const exA = a.veredicto && a.veredicto.excedente
    const exB = b.veredicto && b.veredicto.excedente
    if (exA != null && exB != null && exA !== exB) return exB - exA
 ...
 * diciendo por qué**.
 *
 * ⚠️ **`costoMeta` y `pctTechoMeta` salen de la atribución de META, ⛔ no de la tienda**, igual que las
...
export function totalesVivos(celdas, techo = 0) {
  ...
  return {
    ...
    costoMeta,
    // ⛔ Sin techo cargado es `null` y ⛔ nunca 0: «no se puede juzgar» ⛔ no es «está perfecto».
    // El nombre lleva `Meta` por la misma razón que en `armarZona`: es la vara, y la vara es la
    // compra que Meta se atribuye. En vivo ⛔ no existe la otra — la caja sólo cierra días.
    pctTechoMeta: Number(techo) > 0 && t.compras ? (costoMeta / Number(techo)) * 100 : null,
 ...
 */
export function veredictoDeCelda(celda, { techo = 0, desgaste = null, aprendizaje = null, ventanaJuicio = null } = {}) {
  const porque = []
  const m = veredictoDeCosto(celda.spend, celda.compras, techo)
  const v = m.estado === 'SIN-COMPRAS-ALTO' ? 'ALTO' : m.estado === 'SIN-PRUEBA' ? 'SIN-PRUEBA' : m.estado
  const pctTecho = m.pct
  const pctDiario = celda.diario ? (celda.spend / celda.diasConGasto / celda.diario) * 100 : null
  const medido = { n: m.n, ruido: m.ruido, umbralPct: m.umbralPct, excedente: m.excedente, ventanaJuicio }
  ...
  const avisar = () => {
    ...
    return { porque, pctTecho, pctDiario, ...medido }
  }
  ...
  if (celda.estadoReal === 'sin-avisos') {
    ...
    return { clase: 'rota', titulo: 'Prendida y sin avisos', accion: 'revisar', ...avisar() }
  }
  if (!celda.spend) {
    porque.push('no gastó un peso en la ventana')
    return { clase: 'quieta', titulo: 'No entregó', accion: 'revisar', ...avisar() }
  }
  if (v === '?') {
    porque.push(`gastó ${Math.round(celda.spend)} y compró ${celda.compras}, pero su línea no tiene techo cargado`)
    return { clase: 'sin-techo', titulo: 'Sin techo con qué juzgarla', accion: 'cargar-techo', ...avisar() }
  }
  if (v === 'MIDIENDO') {
  ...
  if (v === 'ALTO') {
    porque.push(`compra a ${Math.round(div(celda.spend, celda.compras))} contra un techo de ${Math.round(techo)} (${Math.round(pctTecho)}%), y son ${m.n} compras: el exceso es más grande que el ruido de esa cantidad (±${Math.round(m.ruido * 100)}%)`)
    if (m.excedente > 0) porque.push(`pagó ${Math.round(m.excedente)} de más que el techo en esta ventana`)
    return { clase: 'alto', titulo: 'Compra arriba del techo', accion: 'pausar', ...avisar() }
  }
  // 🔴 Arriba del techo pero DENTRO del ruido de la muestra. ⛔ No es `ok` —eso afirmaría que
  // rinde— y ⛔ tampoco `midiendo`, que significa «gastó menos que el techo y todavía no compró»:
  // meter acá una pauta de $110.000 que compró 13 veces al 112% colapsaría dos hechos distintos y
  // perdería el número. Y `accion: null` a propósito: lo que hay que mirar lo dice el ORDEN de las
  // filas (por `excedente`), ⛔ no un pill gris en cada renglón.
  if (v === 'SIN-PRUEBA') {
    porque.push(
      `compra a ${Math.round(div(celda.spend, celda.compras))} contra un techo de ${Math.round(techo)} (${Math.round(pctTecho)}%), pero son ${m.n} compras: con esa cantidad ±${Math.round(m.ruido * 100)}% es ruido, así que ${Math.round(pctTecho)}% ⛔ no se distingue de 100%`,
    )
    if (m.excedente > 0) porque.push(`pagó ${Math.round(m.excedente)} de más que el techo en esta ventana`)
    const faltan = comprasParaAfirmar(pctTecho)
    if (faltan != null) {
      porque.push(
        faltan > CONV_APRENDIZAJE
          ? `harían falta ~${faltan} compras para poder afirmarlo ⇒ a este nivel de exceso ⛔ no se va a poder probar mirando esta pauta: la decisión es de presupuesto, ⛔ no de evidencia`
          : `harían falta ~${faltan} compras en la ventana para poder afirmarlo`,
      )
    }
    return { clase: 'sin-prueba', titulo: 'Caro, pero sin suficientes compras', accion: null, ...avisar() }
  }
  // v === 'OK'
...
export function armarZona({
  ...
  // 🔴🔑 **Y LA VENTANA DE JUICIO SE ESTIRA cuando la muestra no alcanza** (5-sep-2026).
  //
  // El guard de arriba resuelve «no juzgar un día suelto». Lo que ⛔ no resolvía es que **siete días
  // tampoco alcanzan** para juzgar una pauta que compró dos veces. Medido contra prod ese día:
  // `TEST BROAD BDI` decía «124%, pausar» sobre 6 compras y en 30 días compra al **56%**;
  // `TEST IP AZUL` decía «144%, pausar» sobre 3 y en 30 días está en **92%**. La pantalla mandaba a
  // apagar dos pautas que rinden.
  //
  // ⇒ cuando el veredicto sale `sin-prueba` (arriba del techo pero dentro del ruido), se vuelve a
  // juzgar con **más días**, y se para en el primer escalón que decide. La fila dice sobre cuántos
  // días la juzgó, porque callarlo dejaría leer un juicio de 30 días como si fuera de la semana.
  //
  // 🔑 **Cuesta cero llamadas**: `COLCHON` (`api/_meta-rendimiento.js`) ya trae 40 días o más, así
  // que las filas ya están acá. Es un recorrido más de algo que ya viajaba.
  //
  // ⛔ **El estiramiento sólo puede BAJAR la alarma, ⛔ nunca proponer escalar.** Treinta días buenos
  // ⛔ no habilitan a subirle plata a algo que la última semana pagó de más: esa asimetría es la
  // misma que la de todo este archivo —pausar es caro de revertir, escalar es barato— aplicada al
  // revés. Un `escalar` que aparece recién al estirar se degrada a `ok`.
  const escalones = ESCALONES_JUICIO.filter((d) => d > juicio)
  const porEscalon = new Map(
    escalones.map((d) => [d, new Map(celdasDeLaFoto(enVentana(filas, desdeDe(cierre, d), cierre)).map((c) => [c.id, c]))]),
  )
  ...
  const celdas = celdasDeLaFoto(deLaVentana).map((c) => {
    ...
    let v = veredictoDeCelda(deJuicio, { techo, desgaste, aprendizaje, ventanaJuicio: juicio })
    // El estiramiento. ⛔ El desgaste NO se recalcula por escalón: compara dos mitades y tiene su
    // propio reloj — a 30 días pediría 60 de serie. Es una señal aparte y así queda.
    if (v.clase === 'sin-prueba') {
      for (const d of escalones) {
        const mas = porEscalon.get(d).get(c.id)
        if (!mas) continue
        const otro = veredictoDeCelda(conConfig(mas), { techo, desgaste, aprendizaje, ventanaJuicio: d })
        v = otro.clase === 'escalar' ? { ...otro, clase: 'ok', titulo: 'Rinde', accion: null } : otro
        if (v.clase !== 'sin-prueba') break
      }
    }
    return {
      ...conHoy,
  ...
  return {
    ...
    totales: {
      ...
      // 🔴🔑 **DOS varas, y la que manda es la de Meta** (5-sep-2026). Hasta acá el total se medía
      // contra los **pedidos reales de Tienda Nube** mientras cada fila se medía contra las
      // **compras que Meta se atribuye**, las dos rotuladas «% del techo» en la misma pantalla: el
      // total decía 81% y las filas 106%, y no había forma de saber a cuál creerle.
      //
      // Lo corrigió Bruno, y la razón es del negocio, ⛔ no del código: *«los pedidos reales pueden
      // ser de otros canales que no sean Meta, por ese motivo, solo tiene que ser META»*. Los 113
      // pedidos de la tienda incluyen orgánico, mail, directo y WhatsApp ⇒ la diferencia contra las
      // 84 compras de Meta ⛔ **no** es «lo que Meta no ve», y usarla para abaratar el costo de la
      // pauta le regalaría plata.
      //
      // ⇒ **`pctTechoMeta` es la vara** —la misma con la que se juzga cada fila— y
      // `pctTechoPedidoReal` queda como referencia de negocio, que es lo que dimensiona el marginal
      // y la elasticidad. ⛔ Nunca se pinta con el tono del techo.
      //
      // ⚠️ El `pctTecho` viejo **se borra en vez de cambiarle el valor**, a propósito: un cliente
      // con el JS cacheado leería `undefined` y la tarjeta dice «sin techo cargado». Degradación
      // visible. Dejarlo con el número nuevo serían dos valores plausibles y ninguno avisando que
      // cambió de significado.
      pctTechoMeta: techo > 0 && t.compras ? (div(t.spend, t.compras) / techo) * 100 : null,
      pctTechoPedidoReal: techo > 0 && pedidos ? (div(t.spend, pedidos) / techo) * 100 : null,
    },
    techo,
    techoCaja,
    objetivoPedidos,
