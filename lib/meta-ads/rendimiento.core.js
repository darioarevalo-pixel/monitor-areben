...
export function elegirVentana(crudo) {
  ...
  if (!DIAS_ZONA.includes(n)) {
    return { error: `«dias» tiene que ser ${DIAS_ZONA.join(', ')} — vino «${crudo}».` }
  }
  return { dias: n }
}
...
export function armarZona({
  ...
  const celdas = celdasDeLaFoto(deLaVentana).map((c) => {
    ...
    const desgaste = desgasteDe(larga.serie, ventana)
    ...
    const conHoy = hoy
      ? { ...c, nombre: hoy.nombre, estado: hoy.estado, estadoReal: hoy.estadoReal, diario: hoy.diario }
      : c
    return { ...conHoy, desgaste, aprendizaje, veredicto: veredictoDeCelda(conHoy, { techo, desgaste, aprendizaje }) }
  })
  ...
  const marginal = marginalEntreVentanas(caja, ventana)
