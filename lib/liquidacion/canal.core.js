...
export function canalDe(nombre) {
  ...
  // 🔑 **El canje va a `tecnica`, y va PRIMERO.** Un canje se factura a $0 (precio de lista + 100 %
  // de descuento), así que contarlo como una venta minorista más hunde el precio promedio de
  // Liquidación con una venta que no fue una venta. ⚠️ Sin esta línea caería en `otro`, que la
  // agrupación cuenta como MINORISTA.
  //
  // 🔴 **`influencer` está acá porque ES el canal de los canjes** (15 en Gestión Nube, ver
  // `CANJE_CHANNEL` en `api/crear-venta.js`): Bruno eligió un canal preseteado porque GN no ofrece
  // en el selector el canal propio que había creado. Consecuencia que hay que saber: **ese canal
  // queda reservado**, y una venta COBRADA que se cargue ahí tampoco va a contar para el promedio.
  // Medido el 1-sep-2026 sobre el espejo entero: no existe ninguna venta con ese canal todavía.
  // `canje` se deja igual, para el día que el canal propio se habilite y para no depender de qué
  // nombre tenga el canal en cada cuenta.
  if (n.includes('canje') || n.includes('influencer')) return 'tecnica'
...
/**
 * Qué canales forman **minorista**. El corte que pidió Bruno el 23-ago-2026: *«mayorista y todo lo
 * que no es mayorista»*, y después, sobre `otro`: *«minorista sería local + online + mercadolibre»*.
 *
 * 🔑 **«Minorista» es una AGRUPACIÓN, no un valor que devuelva `canalDe`.** Meterlo adentro del
 * clasificador sería un segundo criterio de canal viviendo al lado del primero.
 *
 * 🔴 **Vive acá porque ya estaba escrita dos veces**: la declaraba `resultado.ts` (sin un solo uso)
 * y volvió a nacer en `lib/memo/foto.core.js` el 24-ago-2026 sin que nadie lo notara. Dos copias de
 * una regla son dos respuestas la primera vez que una cambia — y ésta va a cambiar el día que
 * Gestión Nube invente un canal.
 *
 * ⚠️ **`tecnica` NO está acá y no es un olvido**: no es una venta (la crea el monitor para descontar
 * stock). Queda fuera de los dos lados del corte, pero **con nombre**: adentro cae también el canal
 * vacío, y evaporarla es el defecto.
 */
export const CANALES_MINORISTA = ['local', 'online', 'otro'];

/**
 * De qué lado del corte cae una venta: `'minorista'`, `'mayorista'` o `null` (técnica o canje).
 *
 * 🔑 **Es `canalDe` + `CANALES_MINORISTA` leídos juntos, ⛔ no un criterio nuevo.** Existe porque el
 * ETL necesita partir cada línea de venta en los dos lados (pedido de Bruno, 17-sep-2026: *«que se
 * pueda sectorizar entre mayorista y minorista, así no se produce un sesgo»* — en BDI el mayorista es
 * el 88 % de las unidades y tapaba lo que compra el público).
 *
 * ⚠️ El `null` ⛔ no es «no sé»: es «no es una venta de ninguno de los dos lados». Quien sume los dos
 * lados y lo compare con el total tiene que contar las técnicas aparte.
 */
export function ladoDeCanal(nombre) {
  const c = canalDe(nombre)
  if (c === 'mayorista') return 'mayorista'
  return CANALES_MINORISTA.includes(c) ? 'minorista' : null
}
