...
/** El orden en que se muestran los talles de letra. Lo demás se ordena por número. */
const ORDEN_LETRA = ['XXS', 'XS', 'S', 'M', 'L', 'XL', 'XXL', 'XXXL'];

/**
 * Los talles de un producto, sacados de los valores de sus variantes de TiendaNube.
 *
 * 🔴 **Los talles ⛔ NO se tipean**, que es lo que hacía la Tabla de talles vieja: un campo de
 * texto que arrancaba con `S, M, L, XL` clavados. Medido el 1-sep-2026 contra la tienda viva: de
 * los 111 productos sin medidas, **sólo 13 tienen eje de talle**. Los otros 98 tienen variantes de
 * color solamente ⇒ una grilla de cuatro columnas les pide inventar tres talles que no existen, y
 * eso es exactamente lo que produce una tabla que dice cosas distintas que el selector de al lado.
 *
 * Lo que no es un talle —los colores— se cae solo: se reconocen las letras conocidas y los números
 * de hasta dos cifras, que es como vienen los de pantalón.
 */
export function tallesDe(variantes) {
  const letras = [];
  const numeros = [];
  for (const v of variantes || []) {
    ...
    if (ORDEN_LETRA.includes(up)) {
      if (!letras.includes(up)) letras.push(up);
    } else if (/^[0-9]{1,2}$/.test(t)) {
      if (!numeros.includes(t)) numeros.push(t);
    }
  }
  letras.sort((a, b) => ORDEN_LETRA.indexOf(a) - ORDEN_LETRA.indexOf(b));
  numeros.sort((a, b) => Number(a) - Number(b));
  // ⚠️ Si conviven los dos ejes gana el que tenga más valores: un producto con talles numéricos y
  // un «M» suelto de una variante mal cargada no tiene que quedar con dos ejes mezclados.
  if (letras.length && numeros.length) return letras.length >= numeros.length ? letras : numeros;
  return letras.length ? letras : numeros;
}
