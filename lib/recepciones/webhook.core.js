...
/**
 * El instante completo, para `confirmada_at`. ⛔ NO reusa `fechaDe`, que recorta a 10 caracteres:
 * dos OC confirmadas el mismo día quedarían empatadas y el orden de la lista volvería a ser el
 * azar del que ordena. Acepta el ISO con hora y también una fecha sola (medianoche).
 */
/**
 * La URL de una foto del renglón. `null` si no es http(s).
 *
 * 🔴 **El filtro por esquema no es decorativo**: esto termina en el `src` de un `<img>` y en el
 * `href` de un enlace de la pantalla. Un `javascript:` en un `href` corre al hacerle clic, y el
 * emisor es otro sistema — lo que llega de afuera no se pone en el DOM sin mirarlo. Se acepta
 * `http` además de `https` para no romper si algún día sirve las fotos sin TLS: hoy las 80 que
 * llegaron son `https`, pero eso es una medición, no un contrato.
 */
const urlDe = (v) => {
  const t = textoDe(v)
  if (!t) return null
...
export function normalizarEvento(payload) {
  ...
  const lineas = crudas.map((l, i) => {
    ...
    return {
      ...
      // 🔑 LAS FOTOS YA VENÍAN. El emisor las manda desde el 1-sep-2026 (`imagen_url` y
      // `imagen_thumb_url` por renglón, en 78 de los 80 renglones de ese día) y el receptor las
      // tiraba, igual que había pasado con `confirmada_at`. ⛔ No hubo que pedirle nada: había que
      // dejar de descartarlo. Las 79 OC del backfill del 27-ago no las traen — son de antes de que
      // él lo prendiera — y por eso la pantalla no puede asumir que siempre hay foto.
      imagen_url: urlDe(li.imagen_url),
      // La chica es la de la grilla. Se guardan las DOS y no se deriva una de la otra: el nombre
      // del archivo lo elige el emisor, y adivinarlo con un `replace` se rompe callado el día que
      // cambie.
      imagen_thumb_url: urlDe(li.imagen_thumb_url),
      en_gn: null,
      producto_id: null,
    }
  })
  ...
  return {
    ...
    oc: {
      ...
      // 🔑 CUÁNDO SE CONFIRMÓ, que es el evento de verdad. Las otras dos las carga una persona y
      // **el emisor las deja vacías en la mayoría de las OC** (62 de las 79 del historial); ésta
      // vino en las 79, sin una excepción. Es por la que se ordena la lista: `recibido_en` no
      // sirve porque un backfill trae todo el historial en el mismo minuto y deja el orden al azar.
      confirmada_at: instanteDe(oc.confirmada_at),
