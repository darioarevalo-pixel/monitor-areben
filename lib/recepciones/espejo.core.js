...
export async function leerEspejo(cliente, lineas) {
  ...
  try {
    ...
    for (const lote of enLotes(skus)) {
      const { data, error } = await cliente.from('inventario').select('sku, barcode, product_id').in('sku', lote)
    ...
    for (const lote of enLotes(barras)) {
      const { data, error } = await cliente.from('inventario').select('sku, barcode, product_id').in('barcode', lote)
      if (error) throw new Error(error.message)
      for (const f of data || []) {
        if (f.barcode) porBarra.set(String(f.barcode), String(f.product_id ?? ''))
      }
