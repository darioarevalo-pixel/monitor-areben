...
/**
 * El resumen de una ronda reducido a lo único que la tarjeta del tablero necesita: `{n, promedio}`.
 *
 * ⛔ **NO devuelve `url` ni `name`**, y eso es el punto entero de que exista. Es la misma disciplina
 * de whitelist que `paraElVotante`, pero acá el motivo no es la privacidad sino el **peso**: el
 * snapshot de la ronda se congela con la `url` tal cual estaba, y los diseños viejos la tienen en
 * base64 (9 de los 37 de BDI, medido el 24-ago-2026). Devolver el resumen "completo" haría que
 * mostrar el ★ **al entrar a la sección** —que es donde tiene que estar— costara esas fotos
 * enteras, en cada entrada, en cada cambio de pestaña y en cada cambio de marca.
 *
 * Se implementa **sobre** `resumen()` a propósito: el día que cambie la aritmética, cambia en un
 * solo lugar y la grilla no se despega del ranking.
 *
 * @param {{ disenos?: any[] }} ronda
 * @param {{ nombre?: string, puntajes?: Record<string, number> }[]} boletas
 * @returns {Record<string, { n: number, promedio: number | null }>}
 */
export function resumenLiviano(ronda, boletas) {
  const salida = {};
  for (const d of resumen(ronda, boletas)) salida[d.id] = { n: d.n, promedio: d.promedio };
  return salida;
}
