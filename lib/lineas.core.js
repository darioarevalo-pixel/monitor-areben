...
/**
 * Las marcas del monitor: las dos bases de Supabase. **Derivada de `LINEAS`, no escrita a mano** —
 * una lista literal acá sería una copia más de `['bdi','zattia']` que se despega el día que aparezca
 * una tercera base, y este archivo existe justo para que «marca» y «línea» tengan un solo dueño.
 *
 * La usa el memo para abrir la venta por canal marca por marca: fusionadas, el local de Zattia
 * (3,9× el de BDI en la semana del 17 al 23 de agosto) se lee como si fuera de BDI.
 */
export const MARCAS = [...new Set(LINEAS.map(baseDeLinea).filter((m) => m !== null))]
