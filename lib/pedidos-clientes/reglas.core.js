...
export const CAMPOS = ['id', 'store', 'texto', 'tipo', 'canal', 'cliente', 'estado', 'nota', 'producto_id', 'sku', 'variante']
...
/**
 * **Cuál es el mismo producto cuando el artículo se ELIGIÓ y no se escribió.**
 *
 * Un faltante puede entrar de dos formas y no valen lo mismo: lo que no trabajamos sólo se puede
 * escribir, pero lo que se acabó se elige del catálogo de Gestión Nube y entonces trae `producto_id`
 * — una identidad exacta, no una parecida.
 *
 * 🔴 **Con `producto_id`, la llave es el id; sin él, el texto.** La consecuencia es que un mismo
 * producto anotado a mano y elegido del catálogo cae en DOS grupos, y se elige a sabiendas: es el
 * mismo criterio que ya eligió `claveDeTexto` con `colores`/`color`. Agrupar de menos deja dos
 * renglones **con casi el mismo nombre a la vista**, y alguien los junta; agrupar de más suma dos
 * productos distintos en un número plausible que nadie tiene con qué contrastar, y ése es el número
 * que hace comprar. Un id nunca junta dos productos; el texto sí puede.
 *
 * ⚠️ Se agrupa por PRODUCTO y no por variante, aunque el sku de la variante se guarde igual: el
 * ranking contesta «qué comprar» —«el corset, 7 veces»—, y partido en cuatro talles de 2 ese
 * producto no aparece en ningún lado. Los talles se leen abajo, en la lista cruda.
 */
export function claveDePedido(p) {
  const id = p && p.producto_id != null ? String(p.producto_id).trim() : ''
  if (id) return `gn:${id}`
  return claveDeTexto(p && p.texto)
}
...
export function filaDe(p, yo, ahora) {
  return {
    ...
    // El artículo elegido del catálogo de GN. Las tres viajan juntas o no viaja ninguna: sin
    // `producto_id` el sku queda sin con qué agrupar, y la variante sola no dice de qué producto es.
    producto_id: p.producto_id ? String(p.producto_id).trim().slice(0, 80) : null,
    sku: p.producto_id && p.sku ? String(p.sku).trim().slice(0, 80) : null,
    variante: p.producto_id && p.variante ? String(p.variante).trim().slice(0, 80) : null,
    creado_por: yo || null,
