/**
 * El FORMATO BASE de una descripción de Zattia, hecho ejecutable.
 *
 ...
 * `generarHtml` acá, igual que `lib/gen-talles/core.ts` arma la tabla: así el estilo vive en
 * un lugar solo y las reglas se prueban sin pintar nada.
 *
 * 🔴 Y por eso `validarBorrador` es carga estructural, no un adorno: cuando el borrador lo
 * escriba un modelo, el JSON Schema de structured outputs NO puede fijar «3 o 4 bullets» ni
 * «máximo 220 caracteres» (no soporta `minItems`/`maxItems`/`maxLength`). Lo único que
 * sostiene el formato es esta función.
 */
...
export const ETIQUETAS = ['Tela', 'Calce', 'Cuello', 'Escote', 'Detalle', 'Largo', 'Manga', 'Espalda'] as const
...
const palabras = (s: string): string[] => sinAcento(s).match(/[a-z0-9]{4,}/g) || []

/** ¿El texto nombra alguna de estas palabras, como palabra entera? */
function nombraAlguna(texto: string, candidatas: string[]): string | null {
  const t = ' ' + sinAcento(texto).replace(/[^a-z0-9]+/g, ' ') + ' '
  for (const c of candidatas) {
    const n = sinAcento(c).replace(/[^a-z0-9]+/g, ' ').trim()
 ...
 * de una, no arreglar uno y descubrir el siguiente.
 */
export function validarBorrador(b: Borrador, ctx: Contexto): Problema[] {
  const problemas: Problema[] = []
  const push = (campo: string, motivo: string) => problemas.push({ campo, motivo })

  const parrafo = (b?.parrafo || '').trim()
  if (!parrafo) push('parrafo', 'está vacío')
  else if (parrafo.length > MAX_PARRAFO) push('parrafo', `tiene ${parrafo.length} caracteres y el máximo es ${MAX_PARRAFO}`)
  ...
  bullets.forEach((bu, i) => {
    const et = (bu?.etiqueta || '').trim()
    const tx = (bu?.texto || '').trim()
    const dónde = `bullet ${i + 1}`
    if (!(ETIQUETAS as readonly string[]).includes(et)) push(dónde, `la etiqueta «${et}» no es una de las admitidas`)
  ...
  const todo = [parrafo, ...bullets.map((x) => x?.texto || '')].join(' ')
  ...
  const color = nombraAlguna(todo, ctx.variantes || [])
  ...
  const tela = bullets.find((x) => (x?.etiqueta || '').trim() === 'Tela')
  if (tela) {
    const fuente = palabras(`${ctx.insumo || ''} ${ctx.nombre || ''}`)
