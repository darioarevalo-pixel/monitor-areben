...
import { ATRIBUTOS } from './atributos.core.js'

/**
 * Cuántas fotos viajan. **Dos** (8-sep-2026): la portada y la que sigue.
 *
 * ⛔ No son todas. Un producto tiene hasta seis y cada una vuelve a pagar la entrada: con dos ya
 * se ve la prenda de frente y de atrás, que es lo que faltaba. El día que haga falta subirlo, el
 * número está acá y en un solo lado.
 */
...
export const SISTEMA = [
  ...
  'Abajo del párrafo la ficha muestra SOLO TRES DATOS —la tela, el tiro y un detalle—, después los cuidados y la tabla de medidas. Nada más. Eso ya está dicho: no lo repitas.',
  '',
  'Todo el resto lo dice TU PÁRRAFO. El corte y cómo cae, el escote o el cuello, las mangas, dónde termina, la estampa, los apliques y herrajes, las costuras que se ven, cómo queda puesta. Nombralos cuando aporten: la clienta no tiene ninguna otra lista donde leerlos.',
  'No los enumeres como una ficha («escote en V, manga corta, largo crop»): escribí una prenda, no un formulario. Y si algo no aporta, dejalo afuera antes que estirar la frase.',
  '',
  'El formato:',
  `- Hasta ${MAX_PARRAFO} caracteres. Si lo que hay para decir entra en una sola oración, escribí una sola oración: un párrafo corto y concreto es mejor que uno largo y vacío.`,
  ...
  '- No escribas el nombre propio del producto: ya está arriba, en el título.',
  '',
  'Las cuatro reglas que no se negocian:',
  '- No repitas los tres datos de abajo —la tela, el tiro y el detalle—, ni con otras palabras. Del resto hacete cargo vos.',
  '- Nada de relleno. «Suma comodidad a tus outfits», «aporta movimiento», «acompaña la figura con soltura», «ideal para toda ocasión» no dicen nada de esta prenda: se podrían pegar en cualquier otra. Si sólo se te ocurre eso, escribí menos.',
  ...
  '',
  'Además del párrafo, escribí un TIP DE LOOK: una sola línea de hasta 120 caracteres sobre cuándo se usa y con qué se combina («levanta un jean de todos los días y también resuelve una salida de noche»). Le corren las mismas reglas: sin colores, sin talles, sin centímetros, y nada que no se vea en la foto. Y dos propias:',
  '- Habla de la OTRA MITAD del look, nunca de lo que va DEBAJO de esta prenda. Si lleva forro o no es un dato que sólo se sabe con la prenda en la mano: no está en la foto ni en los datos, así que «usala con un top debajo» puede ser falso.',
  '- No le prescribas el tiro ni el corte a la otra prenda («con un pantalón de tiro alto»): la tienda vende sobre todo tiro medio y bajo, y un tip así manda a la clienta a un corte que no está. Decilo en general.',
  '',
  '',
  'ADEMÁS DEL TEXTO, UN CHEQUEO DE LA FICHA (`discrepancias`).',
  'Los datos duros los carga una persona en el local, a veces con la prenda en la mano y a veces de memoria: se equivocan. Mirá las fotos y decí qué dato NO coincide con lo que ves.',
  '- Un dato por entrada: `campo` (tela, calce, silueta, escote, manga, largo, tiro o detalle), `dice` (lo que dice la ficha) y `veo` (lo que muestra la foto, en dos o tres palabras).',
  '- Marcá SÓLO lo que se ve con seguridad. Ante la duda, no lo marques: una alarma falsa hace que dejen de mirarlas todas.',
  '- La TELA no se ve en una foto: nunca la marques, salvo que sea imposible (dice «denim rígido» y es un vestido de encaje).',
  '- Si mirás y todo coincide, devolvé la lista vacía. La lista vacía significa «lo miré y está bien», así que no la uses para «no me fijé».',
  '- Vos NO corregís la ficha: la marcás. La corrige una persona.',
  '',
  'Castellano de Argentina, sin signos de exclamación.',
  '- Escribí como habla la clienta, ⛔ no como una revista de moda. Si una palabra no la usa la gente en un local de ropa —«bralette», «must-have», «statement», «oversized», «denim wash»—, no la uses: nombrá las prendas como las nombra el catálogo (top, corpiño, remera, camisa, jean, campera, sweater, short, pollera).',
].join('\n')
...
export const ESQUEMA = {
  ...
  properties: {
    ...
    tip: { type: 'string' },
    // 🆕 EL CHIVATO (8-sep-2026, decisión de Bruno). Va en el ESQUEMA y no en el párrafo porque
    // es un dato, no prosa: la pantalla lo dibuja al lado del campo y ofrece corregirlo. Se pide
    // SIEMPRE por el mismo motivo que el tip: «si ves algo raro, avisá» devuelve silencio.
    discrepancias: {
      type: 'array',
      items: {
        type: 'object',
        ...
        required: ['campo', 'dice', 'veo'],
      },
    },
  },
  required: ['parrafo', 'tip', 'discrepancias'],
}
...
export function armarPedido(ctx, problemas = []) {
  ...
  if (ctx.bullets && ctx.bullets.length) {
    t.push('')
    t.push('Los ÚNICOS datos que están escritos abajo del párrafo. No los repitas ni con otras palabras:')
    for (const b of ctx.bullets) t.push(`- ${b.etiqueta}: ${b.texto}`)
  }
  // 🆕 8-sep-2026: la ficha pasó a ser INSUMO. Lo que ya no se publica igual viaja al prompt —es
  // lo que el local miró con la prenda en la mano— pero como material para escribir, ⛔ no como
  // lista prohibida. ⚠️ Y se le dice de dónde sale y qué hacer si no coincide con la foto: sin
  // eso, un dato mal cargado se convierte en una afirmación del párrafo, que es peor que un
  // bullet visiblemente falso.
  if (ctx.ficha && ctx.ficha.length) {
    t.push('')
    t.push('Lo que el local cargó de esta prenda mirándola. NO se publica: es material para tu párrafo, y podés nombrarlo con tus palabras.')
    for (const f of ctx.ficha) t.push(`- ${f.etiqueta}: ${f.texto}`)
    t.push('Si algo de esto NO coincide con lo que ves en la foto, escribí lo que ves y marcalo en `discrepancias`.')
  }
  ...
  // 🔑 El pedido sale SIN forma de proveedor: `{system, texto, imagenes}` y nada más. La forma
  // de cable —dónde van las fotos, cómo se llama el campo del sistema— la arma el handler. Es lo
  // que hace que cambiar de proveedor sean veinte líneas en un archivo y no una recorrida.
  //
  // 🆕 **Son DOS fotos desde el 8-sep-2026, y ⛔ no es un lujo** (pedido de Bruno). Con una sola
  // —la portada— el modelo no puede ver la espalda ni el ruedo, que es justo donde vive la mitad
  // de lo que hay para describir; y desde que además CHEQUEA la ficha, mirar una sola foto le
  // hace marcar como error lo que simplemente no entra en el cuadro. Medido: la segunda foto
  // ~duplica el costo del borrador (US$0,0015 → ~0,003), o sea US$0,80 el catálogo entero.
  return { system: SISTEMA, texto: t.join('\n'), imagenes: (ctx.imagenes || []).filter(Boolean).slice(0, MAX_FOTOS) }
}

/**
 * Las discrepancias que devolvió el modelo, limpias.
 *
 * 🔴 **`campo` se chequea contra la lista cerrada de atributos, igual que un valor de la ficha.**
 * Lo que contesta el modelo ⛔ no se dibuja crudo: un `campo` inventado sería un aviso que habla
 * de un casillero que no existe y que ningún botón puede abrir — la misma doctrina por la que un
 * valor de ficha lo valida el servidor aunque el `<select>` ya lo haya acotado.
 *
 * ⚠️ Se cortan los textos: son para un renglón al lado del campo, ⛔ no para un párrafo.
 */
export function chivatosDe(x) {
  if (!Array.isArray(x)) return []
  return x
    .map((c) => ({
      campo: String((c && c.campo) || '').trim().toLowerCase(),
      dice: String((c && c.dice) || '').trim().slice(0, 40),
      veo: String((c && c.veo) || '').trim().slice(0, 40),
    }))
    .filter((c) => Object.prototype.hasOwnProperty.call(ATRIBUTOS, c.campo) && c.veo)
    .slice(0, 6)
}
...
export function interpretar(texto) {
  ...
  return {
    borrador: {
      parrafo: d.parrafo,
      tip: typeof d.tip === 'string' ? d.tip.trim() : '',
      chivatos: chivatosDe(d.discrepancias),
    },
  }
}
