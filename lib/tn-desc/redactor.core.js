 ...
 *
 * 🔴 El reintento no es un adorno. El JSON Schema de structured outputs garantiza la FORMA
 * (que venga un `parrafo` y una lista de `{etiqueta, texto}` con la etiqueta de la lista
 * cerrada) y **nada más**: no soporta `minItems`/`maxItems`/`maxLength`, así que «3 o 4
 * bullets» y «hasta 220 caracteres» no se pueden pedir ahí — y «no nombres los colores de
 * ESTE producto» menos todavía. Lo único que sostiene el formato es `validarBorrador`, y
 * cuando rechaza, los problemas vuelven al modelo en vez de tirarse el intento.
 *
 ...
 * la prosa. La pantalla muestra los dos y Bruno compara.
 *
 * ⚠️ Sonnet 5 está con precio de introducción hasta el 31-ago-2026. Se modela con su fecha en
 * lugar de escribir el número de lista: cobrarle de más al comparar empuja la decisión hacia
 * Haiku por un motivo que no es real.
 *
 * ⚠️ `effort` va sólo donde el modelo lo acepta: **Haiku 4.5 rechaza `output_config.effort`
 * con un 400**. Escribir un párrafo de 220 caracteres no necesita esfuerzo alto, así que
 * donde se puede se baja — si no, la comparación no es entre dos modelos sino entre dos
 * configuraciones.
 */
export const MODELOS = {
  'claude-haiku-4-5': {
    nombre: 'Haiku 4.5',
    lista: { entrada: 1, salida: 5 },
    intro: null,
    effort: null,
  },
  'claude-sonnet-5': {
    nombre: 'Sonnet 5',
    lista: { entrada: 3, salida: 15 },
    intro: { hasta: '2026-08-31', entrada: 2, salida: 10 },
    effort: 'low',
  },
}
...
/**
 * Lo que consumió una llamada. Los dos campos de caché van aunque hoy den siempre cero: el
 * día que se cachee el prompt, el costo sigue siendo verdadero sin que nadie se acuerde.
 * @typedef {{entrada:number, salida:number, cacheLeido:number, cacheEscrito:number}} Uso
 */

/** @type {Uso} */
export const USO_CERO = { entrada: 0, salida: 0, cacheLeido: 0, cacheEscrito: 0 }
...
/** Los dólares de una corrida. Leer del caché cuesta ~0,1× la entrada y escribirlo ~1,25×. */
export function costoDe(uso, modelo, hoy) {
  const p = precioDe(modelo, hoy)
  const M = 1_000_000
  return (
    (uso.entrada * p.entrada) / M +
    (uso.cacheLeido * p.entrada * 0.1) / M +
    (uso.cacheEscrito * p.entrada * 1.25) / M +
    (uso.salida * p.salida) / M
  )
}
...
/**
 * El esquema de la respuesta (structured outputs).
 *
 * 🔴 Fija la FORMA y nada más. Los largos y las tres reglas NO se pueden expresar acá — las
 * sostiene `validarBorrador`, y por eso existe el reintento.
 */
...
export function armarPedido(ctx, problemas = []) {
  ...
  const contenido = []
  if (ctx.imagen) contenido.push({ type: 'image', source: { type: 'url', url: ctx.imagen } })
  contenido.push({ type: 'text', text: t.join('\n') })
  return { system: SISTEMA, contenido }
}
