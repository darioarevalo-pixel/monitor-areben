 ...
 *
 * 🔑 Bajó de `formato.ts` a acá el 19-ago-2026 porque el que compone el texto que sale a la
 * tienda es `api/_tn-desc.js`, y un handler de `api/` no puede importar TypeScript. El
 * borrador aprobado se convierte en HTML DEL LADO DEL SERVIDOR, a partir de lo que está
 * guardado en `tn_descripciones` — no de lo que mande el navegador.
 *
 * ⚠️ El `<div>` lleva `max-width:680px`, que es la misma firma con la que el generador de
 * talles reconoce su envoltorio viejo. Por eso el bloque `AREBEN-PROSA` se extrae ANTES de
 * tocar wrappers en las dos puntas: `lib/tn-desc/prosa.ts` acá y
 * `bdi-catalogo/api/_desc-talles.js` del otro lado. Sin eso, medir la prosa da cero y pegar
 * una tabla de talles la borra.
 */
