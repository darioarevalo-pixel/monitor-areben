...
export function validarParrafo(parrafo, ctx) {
  ...
  //    🔴 Y la palabra que ES el tipo de prenda queda AFUERA de esta regla, porque si no las dos
  //    reglas se pelean: la 3ª EXIGE nombrar la prenda y ésta prohibiría nombrarla el día que el
  //    tipo sea, además, el valor de un bullet. Ahí el párrafo NO puede quedar verde nunca —y el
  //    botón de aprobar exige cero problemas—. 📊 Medido el 7-sep-2026 contra el catálogo de
  //    Zattia: le pasa a 20 productos, 18 minis con `Largo: mini` más BUZO BROWN y CAMISA AMELIE.
  //    Es la misma forma que el falso positivo del plural («Jeans» ≠ «jean»): entre una regla que
  //    obliga y una que prohíbe lo mismo, gana la que obliga.
  const delTipo = new Set(palabras(tipo))
  const enParrafo = new Set(palabras(p))
  const repetidas = []
  for (const b of (ctx && ctx.bullets) || []) {
    for (const w of palabras((b && b.texto) || '')) {
      if (delTipo.has(w)) continue
      if (enParrafo.has(w) && !repetidas.includes(w)) repetidas.push(w)
    }
  }
  //    🆕 22-sep-2026: **AVISA, ⛔ no frena.** Bruno: «no deja publicar si se repite algo de la
  //    ficha, pero eso no tiene mucho sentido». La ficha es insumo del párrafo desde el 8-sep, así
  //    que el párrafo nombra la tela o el tiro con toda razón, y el botón quedaba gris por eso.
  //    Se sigue mostrando —y le sigue pidiendo el reintento al modelo— pero ⛔ no bloquea publicar.
  if (repetidas.length) {
    problemas.push({
      campo: 'parrafo',
      motivo: `repite lo que ya dicen los bullets (${repetidas.join(', ')})`,
      aviso: true,
    })
  }
...
/**
 * 🆕 Los problemas que FRENAN aprobar y publicar: todos menos los que son sólo un aviso.
 * 🔑 Es el único lugar que decide qué frena —la pantalla y `scripts/desc-borradores.mjs` preguntan
 * acá— para que el botón y el script ⛔ no puedan discrepar.
 */
export function frenan(problemas) {
  return (problemas || []).filter((x) => !(x && x.aviso))
}
...
export function generarHtml(b) {
  ...
  // El pie de marca va último y sólo si la prenda es de producción propia: lo decide
  // `lib/tn-desc/origen.core.js` desde la orden de compra, ⛔ no se escribe acá.
  if (pie) h += `<p style="font-size:13px;color:#666;margin:16px 0 0;">${esc(pie)}</p>`
  h += '</div>'
