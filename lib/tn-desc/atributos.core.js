 ...
 */
/**
 * Las telas. Viven en una const porque las piden DOS casilleros —`tela` y `tela2`— y una segunda
 * copia sería una lista que se puede desincronizar sin que nadie lo note.
 */
export const TELAS = [
      'microfibra',
      'jersey de algodón',
...
export const ATRIBUTOS = {
  ...
  // 🔑 La SEGUNDA tela (4-sep-2026). No se le pide a ninguna familia: aparece en «+ agregar un
  // dato», porque la mayoría de las prendas tiene una sola y un casillero más para dejar vacío
  // siempre es un casillero que ensucia la ficha. Existe por los CUIDADOS: una falda de
  // microfibra con capa de microtul se cuida como el microtul (FALDA SAGE, el caso de Bruno).
  // ⛔ NO sale como bullet propio: se fusiona con `tela` («Tela: microfibra + microtul»), que es
  // como lo lee la clienta. En la base sigue siendo su propia fila, así que el análisis las ve
  // separadas.
  tela2: { label: 'Segunda tela', orden: 1.5, valores: TELAS },
...
/**
 * «No sé»: **se carga igual, y ⛔ NO sale a la ficha.**
 *
 * 🔴 Nació en `tela` el 27-ago-2026 y el 7-sep-2026 **se abrió a todos los atributos cerrados**,
 * por un pedido de Bruno: *«algunas me parecen demasiado raras, como que no saben qué poner y
 * completan»*. 📊 Y lo que la pantalla ofrecía era el problema: la única salida era «no aplica»,
 * que significa **«esta prenda no tiene eso»** — o sea, AFIRMA. Quien miraba la prenda y no sabía
 * qué cuello era ⛔ no tenía manera de decirlo, así que elegía uno. Un desplegable sin «no sé» ⛔
 * no mide lo que la prenda es: mide lo que la persona se animó a poner.
 *
 * 🔑 Es la MISMA palabra en todos los atributos, ⛔ no una por campo: el día que alguien pregunte
 * «cuántas fichas quedaron sin resolver» es un solo `where valor = 'no identifico'`.
 *
 * 🔑 Y cuenta como CONTESTADO para el `4/6` de la fila, igual que «no aplica»: si no contara, el
 * contador se quedaría corto para siempre y nadie sabría si falta trabajo o si ya se miró.
 */
export const NO_SE = 'no identifico';

/** El mismo valor, con el nombre viejo. `tela` lo trae adentro de su lista desde el 27-ago-2026. */
export const TELA_SIN_IDENTIFICAR = NO_SE;

/**
 * Las telas cargadas de un producto, sin las que no dicen nada.
 *
 * 🔑 Recibe la ficha entera para que quien llama ⛔ no tenga que saber cuántos casilleros de tela
 * existen: el día que haya un tercero, cambia acá y en ningún otro lado. Lo usan el bullet, los
 * cuidados y el freno de publicación.
 */
export function telasDe(cargados) {
  const c = cargados || {};
  return [c.tela, c.tela2].map((x) => String(x || '').trim()).filter((x) => x && x !== TELA_SIN_IDENTIFICAR);
}

/**
 * ⛔ ¿Esta prenda NO tiene tela? Es el freno: sin tela ⛔ no se redacta ni se publica
 * (decisión de Bruno, 4-sep-2026).
 *
 * 🔑 `no identifico` cuenta como sin tela **a propósito**. Se sigue guardando —«lo miró y no
 * supo» es distinto de «nadie lo cargó», y esa diferencia es la que dice qué prenda hay que
 * volver a mirar—, pero ⛔ no alcanza para salir a la tienda.
 */
export function sinTela(cargados) {
  return telasDe(cargados).length === 0;
}
...
export function propuestasDe(familia, cargados) {
  ...
  for (const a of [...atributosDe(familia), ...atributosExtra(familia)]) {
    ...
    if (!valor || valor === NO_APLICA || valor === NO_SE) continue;
 ...
 */
/**
 * ⛔ **`silueta` salió de las tres familias que la pedían el 8-sep-2026, y ⛔ no se borró.**
 *
 * 📊 Medido sobre las **207 prendas de arriba** con ficha cargada: **67 % `regular` y 27 % `no
 * aplica` — el 94 % entre dos valores que ⛔ no distinguen nada**, contra 12 `oversize` en total.
 * Un desplegable que en 19 de cada 20 prendas contesta lo mismo ⛔ no es un dato: es un casillero
 * que hay que atender, y atenderlo cuesta en el único lugar donde el tiempo importa —el local, con
 * la prenda en la mano—.
 *
 * 🔑 **Sigue existiendo como atributo**: las 207 que ya lo tienen cargado lo muestran, y quien
 * necesite declarar la silueta de una prenda puntual la suma con «+ agregar un dato». Lo que
 * cambió es que ⛔ ya no se le pregunta a TODAS.
 */
export const FAMILIAS = {
  tops: {
    label: 'Tops y bodies',
    categorias: ['TOPS Y BODIES', 'REMERAS', 'BLUSAS', 'BLUSAS Y CAMISAS', 'CORSETS'],
    atributos: ['tela', 'calce', 'escote', 'manga', 'largo', 'detalle'],
    valores: {
...
export function opcionesDe(familia, atributo) {
  ...
  return {
    ...
    // ⛔ En `tela` no se suma acá porque ya viene adentro de `TELAS`: ofrecerlo dos veces sería
    // el mismo valor repetido en el mismo desplegable.
    noSe: atributo !== 'tela',
  };
}
...
export function esValor(familia, atributo, valor) {
  ...
  // «No sé» vale en todos: en `tela` porque está en su lista, en el resto por el 7-sep-2026.
  if (v === NO_SE) return true;
 ...
 *
 * 🔴 **Se decide PALABRA POR PALABRA y ⛔ no sobre el valor entero.** El primer intento miraba el
 * valor completo y se cayó con el primer caso real: Bruno corrigió a mano
 * `DETALLE EN EL BOLSILLO trasero`, y esa única minúscula dejaba las otras cuatro palabras
 * gritando. Una corrección a medias es exactamente lo que va a pasar siempre, porque lo que ya
 * está cargado viene en mayúscula y lo que se agrega después se escribe normal.
 *
 * ⛔ Y una palabra baja sólo si tiene **dos letras o más**: «cuello V» y «escote V» son la letra
 * que nombra la forma, no un grito. «100%» no tiene letras y queda igual.
 */
function sinGritar(v) {
  return v
    .split(' ')
    .map((w) => ((w.match(/[A-ZÁÉÍÓÚÜÑ]/g) || []).length >= 2 && !/[a-záéíóúüñ]/.test(w) ? w.toLocaleLowerCase('es') : w))
    .join(' ');
}
...
/**
 * 🆕 **Los ÚNICOS atributos que SALEN A LA TIENDA** (8-sep-2026, decisión de Bruno).
 *
 * 🔴 La pregunta que lo abrió fue suya: *«calce, cuello, manga y largo, ¿es necesario? porque todo
 * eso se ve en la foto… la ficha estuvo pensada para ARMAR la descripción, ⛔ no para estar
 * EXPUESTA en la descripción»* ⇒ **la ficha queda como INSUMO del párrafo.**
 *
 * 🔑 **El criterio ⛔ no es «qué importa»: es qué NO contesta la foto.**
 * - `tela` — una foto de estudio ⛔ no distingue una gasa de un voile, y de ella salen los cuidados.
 * - `tiro` — es el atributo más escrito de la tienda (41 menciones de las 328 fichas viejas) y ⛔
 *   la foto ⛔ no lo dirime: revisando las 17 el 8-sep hubo que recortar la foto de JEAN INDIGO y
 *   aun así quedó en duda. Si con la foto delante ⛔ no se puede decidir, la clienta tampoco.
 * - `detalle` — es el campo libre: dice lo que ningún otro campo dice.
 *
 * ⛔ **`calce`, `silueta`, `escote`, `manga` y `largo` ⛔ NO se publican**, y ⛔ no es que no
 * importen: se VEN en la foto, y son los cinco donde apareció **el 100 % de los errores de ficha**
 * que se cazaron revisando las 17 (`musculosa` en tres prendas halter, `asimétrico` en un top
 * simétrico, `redondo` en un cuello alto). Un dato que la foto contradice ⛔ no ayuda: confunde.
 *
 * 🔑 **Se siguen CARGANDO igual.** Alimentan el párrafo, y el día que exista la pantalla que suma
 * el catálogo —«qué escote se vendió más»— van a estar. Cargar y publicar son dos decisiones.
 */
export const SE_PUBLICAN = ['tela', 'tela2', 'tiro', 'detalle'];
...
export function bulletsDe(familia, cargados) {
  const out = [];
  // 🔑 La tela sale PRIMERO y de un solo bullet, con las dos adentro si hay dos. Se resuelve
  // afuera del bucle porque son dos casilleros que se dibujan como uno, y porque `tela` es el
  // atributo de orden 1 en las cinco familias: emitirlo acá no cambia el orden de nada.
  const telas = telasDe(cargados).filter((t) => esValor(familia, 'tela', t));
  if (telas.length) out.push({ etiqueta: ATRIBUTOS.tela.label, texto: sinGritar(telas.join(' + ')) });
  ...
  for (const a of campos) {
    const valor = String((cargados || {})[a.key] || '').trim();
    // Las dos telas ya salieron juntas, arriba.
    if (a.key === 'tela' || a.key === 'tela2') continue;
    // 🔴 El filtro que hace que la ficha sea INSUMO y ⛔ no vidriera. Ver `SE_PUBLICAN`.
    if (!SE_PUBLICAN.includes(a.key)) continue;
    // ⛔ «No aplica» y «no identifico» se guardan y NO salen: son datos de carga, no de venta.
    // La diferencia entre los dos es la que dice si hay que volver a mirar la prenda.
    if (valor === NO_APLICA || valor === NO_SE) continue;
...
/**
 * 🆕 **La otra mitad de la ficha: lo que se cargó y ⛔ NO se publica** (8-sep-2026).
 *
 * 🔑 Es exactamente el complemento de `bulletsDe`, y existe porque esos valores **no dejaron de
 * servir: dejaron de ser vidriera**. Van al prompt como material para escribir el párrafo —el
 * local los miró con la prenda en la mano— en vez de irse a la basura.
 *
 * ⚠️ Se arma con la misma lista y el mismo orden que los bullets, a propósito: si un día alguien
 * mueve un atributo de un lado al otro, la única cosa que hay que tocar es `SE_PUBLICAN`.
 */
export function insumosDe(familia, cargados) {
  const out = [];
  const campos = [...atributosDe(familia), ...atributosExtra(familia)]
    .filter((a) => String((cargados || {})[a.key] || '').trim())
    .sort((a, b) => a.orden - b.orden);
  for (const a of campos) {
    if (SE_PUBLICAN.includes(a.key)) continue;
    const valor = String((cargados || {})[a.key] || '').trim();
    // ⛔ Mismos dos descartes que en los bullets: son respuestas de carga, ⛔ no datos de la prenda.
    if (valor === NO_APLICA || valor === NO_SE) continue;
    out.push({ etiqueta: etiquetaDeBullet(a.key, valor), texto: textoDeBullet(a.key, valor) });
  }
  return out;
