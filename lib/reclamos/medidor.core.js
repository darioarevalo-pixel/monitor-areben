 ...
 *
 * ⚠️ El `@param` va con el prefijo del objeto (`p.reclamos`) y ⛔ no suelto: sobre un parámetro
 * **desestructurado**, un `@param {object[]} reclamos` le dice a TypeScript que el objeto ENTERO es
 * un `object[]`, y todos los llamadores tipados se ponen rojos por una propiedad que sí existe.
 *
 * @param {object} p
 * @param {object[]} p.reclamos filas de `devoluciones` con `estado` y `created_at`
 * @param {object[]} p.ventas   filas de `ventas` con `channel` y `date_sale`
 * @param {string[]} p.meses    los meses a dibujar, del más viejo al más nuevo. **Obligatorio.**
 * @param {string|null} p.desdeQueSeRegistra mes (`YYYY-MM`) del PRIMER reclamo que registró la base,
