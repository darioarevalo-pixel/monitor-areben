...
/**
 * **Las unidades de más que salieron de OTRA venta y todavía no dicen de cuál.**
 *
 * 🔑 Es el único caso del módulo que toca **dos ventas**: al cliente le llegó un producto que no
 * compró, así que del otro lado hay una venta a la que le falta ese producto y **un cliente que
 * todavía no reclamó**. La pantalla ya decía «se guarda cuál y se avisa» y ⛔ no se guardaba nada:
 * el escenario `otra_venta` era una etiqueta y el faltante de la otra venta dependía de que alguien
 * se acordara — o de que el otro cliente llamara.
 *
 * ⚠️ Sólo `otra_venta`. En `sin_identificar` **no se puede** anotar cuál es (para eso está el
 * escenario), y ⛔ exigirlo sería el pendiente que nadie puede tildar nunca: el modo de falla que
 * este módulo ya tuvo dos veces. En `de_nadie` no hay otra venta.
 *
 * Va **por unidad** por lo mismo que `destino` y `baja_at`: dos productos de más pueden venir de
 * dos ventas distintas, y una columna sola no lo puede decir.
 */
export function sinLaOtraVenta(fila) {
  const campo = deDondeVuelve(fila && fila.motivo);
  if (!fila || fila.motivo !== 'excedente' || fila.escenario !== 'otra_venta') return { campo, unidades: [] };
  const lista = Array.isArray(fila[campo]) ? fila[campo] : [];
  const unidades = [];
  lista.forEach((item, i) => {
    if (item && String(item.otra_orden == null ? '' : item.otra_orden).trim()) return;
    unidades.push({ i, item });
  });
  return { campo, unidades };
}

/**
 * Anota de qué otra venta salió el producto de más. `indices` en `null` = todas las que faltaban.
 *
 * ⚠️ Volver a anotar una que ya tenía número ⛔ no lo pisa, igual que `recibirUnidades` y
 * `descontarUnidades`: el primero es el dato, y pisarlo borraría a qué venta se fue a mirar.
 */
export function anotarLaOtraVenta(fila, indices, orden) {
  const campo = deDondeVuelve(fila && fila.motivo);
  const lista = Array.isArray(fila && fila[campo]) ? fila[campo] : [];
  const numero = String(orden == null ? '' : orden).trim();
  const pendientes = new Set(sinLaOtraVenta(fila).unidades.map((u) => u.i));
  // `Number` como en `recibirUnidades`: los índices llegan del JSON del POST y un "0" de texto no
  // matchea un 0 en un Set — anotaría cero unidades sin decir por qué.
  const elegidas = indices == null
    ? pendientes
    : new Set(indices.map((n) => Number(n)).filter((n) => pendientes.has(n)));
  let anotadas = 0;
  const nueva = lista.map((item, i) => {
    if (!numero || !elegidas.has(i)) return item;
    anotadas++;
    return { ...item, otra_orden: numero };
  });
  const faltan = sinLaOtraVenta({ ...fila, [campo]: nueva }).unidades.length;
  return { campo, lista: nueva, anotadas, faltan };
}
