 ...
 *           monto: number|null, forma: string|null, diferencia: number|null }} o
 * @returns {{ error?: string, campos?: Record<string, unknown> }}
 */
export function camposAlContestarLaOferta(o) {
  const { respuesta, motivo, escenario, monto, forma, diferencia } = o;
  ...
  if (respuesta === 'rechazo') {
    return { campos: { retencion_respuesta: 'rechazo' } };
  }
  ...
  return {
    campos: {
      ...
      via_retorno: null,
      destino_prenda: destinoDe(motivo, false, escenario),
      estado: 'resuelto',
