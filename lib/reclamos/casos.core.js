...
// ── Soltar la decisión: qué se borra y qué NO ───────────────────────────────────

/**
 * **Los campos que deja `liberar-decision`** (B3, decisión de Bruno del 30-ago-2026).
 *
 * La línea es **qué se DECIDIÓ contra qué se MIDIÓ**: soltar una decisión tiene que dejar la fila
 * como si nadie la hubiera tomado, ⛔ pero no tirar los datos del caso —el flete que cotizamos, el
 * PVP de feria que alguien fue a mirar, lo que el cliente recibió por error—, porque esos ⛔ no son
 * la decisión: son lo que hace falta para volver a tomarla, y hacerlos tipear de nuevo es la misma
 * fricción que ya tuvo este módulo con el envío (*«no puedo salir del envío»*, 27-ago-2026).
 *
 * 📊 **Medido antes de elegir, y es lo que hizo la respuesta barata**: `DecidirReclamo` ⛔ **no
 * prefila NINGUNO** de los campos que se borran acá —los ocho `useState` que leen la fila son
 * `escenario`, `expectativa`, `envio_costo`, los tres de retención, `items_correctos` y
 * `via_retorno`—, así que limpiarlos ⛔ no le saca nada a nadie.
 *
 * 🔑 **La oferta de retención se queda, y ⛔ no es una excepción: es B1.** *«Se parte en dos: armar
 * una oferta exige la decisión, contestarla siempre se puede»* — un «no aceptó» es un hecho que ya
 * pasó, y el rato entre soltar la decisión y contestar la oferta es justo el que
 * `retencion_respuesta` vino a dejar de perder.
 *
 * ⚠️ **`destino_prenda` se borra JUNTO con el destino por unidad del jsonb.** `destinoDeUnidad` es
 * `item.destino || fila.destino_prenda`, así que borrar sólo la columna dejaría a las unidades que
 * tienen el suyo contestando con el destino de una decisión que ya nadie sostiene, y a las que no
 * lo tienen contestando `null`: **media decisión borrada**, que es la forma exacta del defecto que
 * este módulo ya arregló dos veces.
 *
 * ⚠️ **`devolver_envio` es `not null default false`** ⇒ va a `false` y ⛔ no a `null`.
 * `retorno_sugerido` y `retorno_decidido` sí son nullables, y `null` es lo que corresponde: ⛔ no es
 * «no vuelve», es **sin contestar**.
 *
 * 🔑 **Hoy ninguno de estos campos se lee con la decisión soltada** —los cuatro caminos vivos están
 * gateados por `compensacion` (el mensaje de resolución), por los pendientes (el KPI de plata), por
 * `estado` (la bandeja de retornos) o escriben todo de nuevo (`camposAlContestarLaOferta`)—. Eso
 * ⛔ **no es un argumento para dejarlos**: es lo que hace que borrarlos ⛔ no rompa nada hoy. Una
 * columna que dice *«lo que costó el caso»* sobre un reclamo sin decisión **afirma**, y el próximo
 * que lea esa columna directamente ⛔ no va a saber que tenía que preguntar por `compensacion`.
 */
export function camposAlSoltarLaDecision(fila) {
  const sinDestino = (lista) => (Array.isArray(lista) ? lista.map((it) => {
    if (!it || it.destino == null) return it;
    const { destino, ...resto } = it;
    return resto;
  }) : lista);
  return {
    // Lo que se decidió: se va entero.
    compensacion: null,
    estado: 'en_revision',
    monto_total: null,
    monto_producto: null,
    monto_acordado: null,
    monto_envio_devuelto: null,
    costo_caso: null,
    retorno_sugerido: null,
    retorno_decidido: null,
    devolver_envio: false,
    destino_prenda: null,
    via_retorno: null,
    cupon_codigo: null,
    items: sinDestino(fila && fila.items),
    items_correctos: sinDestino(fila && fila.items_correctos),
    ...pendientesDe({ compensacion: null }),
  };
}
