 ...
 */
/**
 * **Lo facturado de una venta**: mercadería, menos el descuento, más el envío. La base imponible.
 *
 * 🔑 **Salió afuera de `cascadaDeVenta` el 23-ago-2026** porque apareció el segundo lector: la
 * serie diaria de `lib/ventas-diarias/core.js` muestra la plata de cada día por canal y **no**
 * necesita el resto de la cascada (IVA, comisiones, CMV) — pero sí necesita que «la plata» quiera
 * decir exactamente lo mismo que en Dirección. Escrita de nuevo allá serían dos números para la
 * misma pregunta, que es de lo que Norte salió a rescatar al negocio.
 *
 * ✅ **Cotejado contra `ventas.total_price`**, que es el total que trae Gestión Nube y llega por
 * otro camino: 1.006 de 1.016 ventas de BDI y 739 de 751 de Zattia coinciden al peso en 37 días
 * (23-ago-2026), y las que no difieren en $10 a $42 por redondeo — $79 sobre $44,7 M en BDI y $170
 * sobre $25,2 M en Zattia. ⛔ Por eso `total_price` **no** se usa como fuente: sería un segundo
 * origen de la misma cifra que casi siempre concuerda, que es la peor forma de tener dos números.
 * Se usa de **oráculo**, que es para lo que sirve venir por otro camino.
 *
 * @param venta       fila de `ventas`: `discount`, `shipping_cost`
 * @param mercaderia  la suma de `venta_detalles.total` de esa venta (o de la parte que se mira)
 */
export function facturadoDeVenta(venta, mercaderia) {
  return mercaderia - num(venta.discount) + num(venta.shipping_cost)
}
...
export function cascadaDeVenta({ venta, mercaderia, tipo, pct }) {
  ...
  const base = facturadoDeVenta(venta, mercaderia)
