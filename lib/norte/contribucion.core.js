...
export function contribucionPorCanal({ ventas, detalles, cuentas, comisiones, desde, hasta }) {
  ...
  // Las unidades y la mercadería son por línea; el resto de la cascada es por venta. Se acumulan
  // los detalles primero para no recorrerlos una vez por venta.
  ...
  for (const v of ventas || []) {
    ...
    const mercaderia = linea.mercaderia
    const descuento = num(v.discount)
    const envio = num(v.shipping_cost)
    ...
    const comision = (mercaderia - descuento + envio) * pctDe(v.payment_method)

    a.ventas++
    a.unidades += linea.unidades
    a.mercaderia += mercaderia
    a.iva += iva
    a.envios += envio
    a.descuentos += descuento
    a.netas += base - iva
    a.cmv += num(v.total_cost)
    a.comisiones += comision
