 ...
 *
 * Cinco formas, y sólo la cuarta elige gente:
 *
 ...
 *   { tipo: 'horas-extras' }                 — a quien tenga el tilde «hace horas extras».
 *
 ...
 *
 * # La quinta llegó por las horas extras, el 30-ago-2026
 *
 * La rutina mensual «Cargar las horas extras» estaba cargada con los tres nombres escritos a mano.
 * Funcionaba, y esconde el problema que tiene toda lista escrita a mano: **son dos verdades sobre
 * lo mismo**. El día que alguien empieza —o deja— de hacer horas extras hay que acordarse de venir
 * a editar esta rutina, y el día que no se acuerden la alerta le va a estar pidiendo horas a quien
 * no las hace, o callándose con quien sí.
 *
 * ⇒ El dato pasó a ser **del perfil** (`perfil.horasExtras`, un tilde en Usuarios, al lado del
 * link de carga) y el destino se DERIVA, exactamente como `{tipo:'seccion'}` se deriva de los
 * permisos. Hay un solo interruptor y no puede desincronizarse de sí mismo.
 *
 * 🔑 **Por qué no fue un rol ni un sub-permiso**, que eran las dos formas que ya existían: un rol
 * (`Funcion`) es un SECTOR y arrastra acceso a pantallas (`ACCESO_POR_FUNCION`); un sub-permiso
...
export function normalizarDestino(d) {
  ...
  // No lleva lista que validar: la lista la contesta el padrón en `esParaMi`. Por eso tampoco hay
  // un caso "vacío" que caiga a todos — acá el vacío es un estado legítimo del equipo (hoy nadie
  // hace horas extras), no una carga a medio terminar.
  if (d.tipo === 'horas-extras') return conMarca({ tipo: 'horas-extras' }, marca);
...
export function esParaMi(destino, perfil) {
  ...
  // 🔴 **Y las horas extras van ACÁ ARRIBA por el mismo motivo**: esto también tiene dueña. Si lo
  // recibiera el admin por ser admin, el «Hoy» de Bruno tendría todos los fines de mes un pendiente
  // que no es suyo, y —peor— podría tildarlo, dando por cargadas las horas de otra. El admin lo
  // recibe si y sólo si se tilda a sí mismo, que es la respuesta correcta a "¿hacés horas extras?".
  // ⚠️ Misma consecuencia que el destino por nombre: `paraMi` es el candado del tilde, así que el
  // admin ve la rutina en Rutinas y en Cumplimiento, la edita y la elimina, pero no la tilda.
  if (d.tipo === 'horas-extras') return perfil.horasExtras === true;
