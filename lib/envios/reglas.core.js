...
/**
 * **El paquete llegó pero la plata no**: el cadete lo entregó y marcó «No cobré» desde la puerta.
 *
 * 🔑 **`null` no es `false`.** `null` es "no dijo nada" —el 100% de las filas anteriores al portal, y
 * todas las que se tildan desde la pantalla interna— y se salda de la forma opuesta: se da por
 * cobrado, que es lo normal en la calle. Tratarlo como `false` daría vuelta la cuenta histórica
 * entera de un día para el otro y haría aparecer una deuda con las clientas que no existe.
 *
 * Sólo aplica a los entregados: en un `no_entregado` no se cobró porque no hubo puerta, y ése ya
 * está afuera de toda la cuenta.
 */
export function cobroPendiente(e) {
