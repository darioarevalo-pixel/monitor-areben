...
/**
 * **La hora de la entrega, que la escriben los dos lados.**
 *
 * La marca el que toca «Entregado»: la pantalla interna (`action:'estado'`) y el portal del cadete,
 * que es el que la toca de verdad, en la puerta. Vive acá y no en cada handler por lo de siempre —
 * dos copias de la misma regla se corrigen de a una.
 *
 * 🔴 **Volver atrás la BORRA.** Si alguien corrige un `entregado` a `no_entregado`, esa hora deja de
 * ser cierta: dejarla puesta sería el registro de una entrega que no pasó, y encima con hora exacta,
 * que es lo que la vuelve creíble. Devolver siempre el campo —con la hora o con `null`— es lo que
 * hace que no haga falta acordarse de limpiarlo.
 *
 * 🔑 **No se deduce de `updated_at`**: esa columna se mueve con cualquier escritura, así que corregir
 * el precio de un envío entregado la semana pasada le cambiaría la hora de entrega. Un dato que
 * parece bueno y envejece mal es peor que no tenerlo.
 */
export function selloDeEntrega(estado, ahora) {
  return { entregado_en: estado === 'entregado' ? ahora : null };
}
...
export const CAMPOS =
  ...
  'localidad, cp, anotacion, monto_envio, envio_pagado, envio_bonificado, monto_pedido_a_cobrar, estado, entregado_en, ' +
...
export const CAMPOS_CUENTA =
  ...
  'envio_bonificado, monto_pedido_a_cobrar, cobrado, entregado_en';
...
/**
 * El parche que saca un envío del día, y **reabre el que ya estaba entregado**.
 *
 * 🔴 Sin esto, sacar del día un `entregado` lo dejaba preso: cae en la bandeja porque
 * `FILTRO_BANDEJA` pregunta `fecha is null` sin mirar el estado, pero un cerrado no dibuja el botón
 * de estado (`siguienteEstado` devuelve `null`) ni lleva el cartel de «volvió», que es sólo de
 * `no_entregado`. Quedaba una fila sin un solo verbo encima: la única salida era el tacho.
 *
 * 🔴 **`no_entregado` NO se reabre, y es el caso que parece igual y es el opuesto.** Ése vive en la
 * bandeja a propósito, con su cartel y con el rastro de su intento fallido, y `agendar` cuenta con
 * encontrarlo así para apilar el intento (`conIntentoFallido`). Reabrirlo acá borraría los dos.
 *
 * 🔑 **Se va el sello de entrega y el tilde de cobro** por la misma regla que ya rige «Corregir»:
 * volver atrás una entrega borra su hora. Una hora exacta sobre un envío que se sacó del día es el
 * registro preciso de algo que no pasó, y la plata la cuenta sólo mientras la fila tiene día.
 *
 * Los tres campos de la reapertura van **opcionales y no siempre presentes**: escribirlos en `null`
 * cuando no corresponde sería pisar el sello de una entrega que nadie deshizo.
 *
 * @param {{ estado?: string } | null | undefined} actual
 * @returns {{ fecha: null, turno: null, estado?: string, entregado_en?: null, cobrado?: null }}
 */
export function parcheAlSacarDelDia(actual) {
  const parche = { fecha: null, turno: null };
  if (actual && actual.estado === 'entregado') {
    parche.estado = 'pendiente';
    parche.entregado_en = null;
    parche.cobrado = null;
  }
  return parche;
}
 ...
 *
 * Se le pasan **todos** los envíos con fecha y todos los movimientos, sin filtrar por rango: el
 ...
 *
 * ⛔ **Los CIERRES DE DÍA se fueron el 17-ago-2026 y eran la tercera fuente de días.** «Cerrar el
 * día» no cambiaba ningún número —el saldo sale de los envíos entregados y de los movimientos, y un
 * día cerrado o abierto daba exactamente lo mismo— así que era un paso que sólo producía un KPI y
 * una línea que decía quién lo había apretado. 🔴 **Y nadie lo apretó nunca: `envios_dia` quedó vacía
 * desde el 13-ago**, o sea que el sistema funcionó un mes entero sin ese paso. Lo notó Bruno usándolo
 * y se sacó. Había perdido su razón de ser en la tanda G, cuando la plata del día (`trajo`) se mudó a
 * `envios_movimientos`: de ahí en más era «alguien lo revisó», que es lo que siempre fue en la
 * práctica. ⚠️ Su caso propio —un día sin reparto en el que igual se movió plata— **ya lo cubren los
 * movimientos**, que tienen fecha propia; por eso sacarla no deja ningún día afuera.
 */
export function cuentaDelCadete(envios, movimientos = []) {
  ...
  // 🔑 **Los días salen de DOS fuentes, y la segunda es la que importa.** Un movimiento puede caer
  // en un día sin reparto —el jueves que rinde lo del lunes al miércoles, el sábado que se le
  // transfiere lo que se le debía— y ése es justamente el caso que la tabla vieja no sabía anotar: su
  // PK era el día de reparto. Sin esta línea, esa plata existe en la base y no aparece en ninguna
  // fila. (Hubo una tercera fuente, los cierres de día; ver el final del docblock.)
  ...
  for (const fecha of fechas) {
    ...
    dias.push({
      ...
      tarifas,
      debeTraer,
      // 🔑 **Los envíos del día enteros, para el detalle que se abre con el `+`.** Van los del día y
      // no sólo los entregados: «3 de 4» sin poder ver cuál fue el que no llegó es justo la pregunta
      // que se hace mirando ese número. ⛔ **No viajan al portal**: `paraElCadeteCuenta` arma la
      // salida campo por campo y su juego de campos está afirmado como lista cerrada.
      filas: delDia,
