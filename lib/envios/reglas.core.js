...
/**
 * Lo que se lee de un cierre de día.
 *
 * ⚠️ **Sin `trajo` ni `pagado_aparte`.** Se fueron con la tanda G y la plata del día vive en
 * `envios_movimientos`; no se piden a propósito ni mientras las columnas sigan en la base.
 *
 * 🔑 **Vive acá desde el 17-ago-2026, no adentro de `api/_envios.js`**: la cuenta la arman **dos**
 * handlers —la pantalla interna y el portal del cadete— y una lista de columnas copiada es el modo de
 * falla propio de este módulo (una columna que no se pide llega `undefined` y **no falla nada**). Es
 * el mismo motivo por el que están acá `CAMPOS`, `CAMPOS_CUENTA` y `CAMPOS_MOVIMIENTO`.
 */
export const CAMPOS_CIERRE = 'fecha, nota, cerrado_por, cerrado_en';
