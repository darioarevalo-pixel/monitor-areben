 ...
 */
export const OFFSET_AR_MS = -3 * 3600000;
...
// ── La cuenta, vista desde el teléfono del cadete ────────────────────────────────────────────

/**
 * Cuánta historia de la cuenta viaja al teléfono.
 *
 * 🔴 **El saldo se calcula sobre TODOS los días; esto recorta sólo el DETALLE.** Son dos cosas
 * distintas y confundirlas es el mutante que importa: `cuentaDelCadete` arrastra el acumulado desde
 * el principio, así que filtrar los envíos **antes** de llamarla da un saldo distinto según por dónde
 * se empiece a mirar — y sería un número plausible, que es lo peor que puede pasar con la plata.
 *
 * 🔑 **Por qué se recorta igual**: esto es lo único de Envíos abierto a internet. Un año de rendiciones
 * no le sirve a nadie arriba de una moto, y lo que no viaja no se filtra. Dos meses cubren cualquier
 * discusión real («¿lo del jueves pasado te lo pagué?»).
 */
export const DIAS_DE_CUENTA = 60;

/**
 * **La cuenta del cadete, armada para su teléfono.**
 *
 * 🔴 **Campo por campo, como `paraElCadete`.** Los días que devuelve `cuentaDelCadete` traen adentro
 * los movimientos crudos de la base, y las filas de envíos con las que se calcularon traen `cliente`
 * y `orden_numero` (ver `CAMPOS_CUENTA`). Un `{...dia}` mandaría a internet nombres de clientas para
 * dibujar una tabla de plata que no los usa — y el día que alguien agregue una columna a
 * `envios_movimientos`, saldría sola.
 *
 * 🔑 **Los días son AGREGADOS**: cuántos envíos, cuántos entregados y los cuatro números. Ni un dato
 * de una clienta entra en esta vista, que es lo que la hace mucho menos cara que la hoja del día.
 *
 * 🔑 **La línea de «saldo anterior» no es decoración**: sin ella el primer día visible arranca con un
 * acumulado que no se deduce de nada de lo que está en pantalla, y la resta no cierra. Es lo que
 * permite recortar sin mentir.
 *
 * 🔴 **En el papel nunca va un número con signo, y acá tampoco**: el verbo y el monto en positivo
 * salen de `claseDelMovimiento` y `MOVIMIENTO_LABEL`, las mismas del recibo impreso. `nota` y `autor`
 * viajan porque el recibo que él ya tiene en la mano los dice: esconderlos en la pantalla dejaría dos
 * comprobantes del mismo movimiento que no se pueden cotejar.
 */
export function paraElCadeteCuenta(cuenta, hoyAR) {
  const todos = (cuenta && cuenta.dias) || [];
  const desde = esFechaIso(hoyAR) ? sumarDiasIso(hoyAR, -DIAS_DE_CUENTA) : null;
  // 🔑 **Un día sin entregas ni movimientos no tiene nada que decirle a él, y se va.** Se vio abriendo
  // la pantalla en prod, no en un test: un envío agendado para pasado mañana crea un día en la cuenta
  // —la lista sale de las tres fuentes— y en el teléfono eso es una fila que dice «mié 19-ago · saldo
  // $0» y nada más. En la pantalla interna esa fila sí sirve (es la vista operativa); acá es ruido.
  // ⚠️ Se filtra DESPUÉS de acumular y sólo de lo visible: un día así aporta cero al saldo, así que
  // sacarlo de la lista no mueve ningún número. Filtrar antes sí lo movería.
  const visibles = todos.filter((d) => (desde ? d.fecha >= desde : true) && (d.entregados > 0 || (d.movimientos || []).length > 0));

  // El saldo con el que arranca lo que se ve: el acumulado del último día que quedó afuera. Sale de
  // la cuenta ya calculada, no de una segunda suma que pueda no coincidir con la primera.
  const previos = desde ? todos.filter((d) => d.fecha < desde) : [];
  const saldoAnterior = previos.length ? num(previos[previos.length - 1].acumulado) : 0;

  return {
    // El saldo de hoy es el de la cuenta ENTERA, dicho con la misma frase que el recibo.
    saldo: rotuloDeSaldo(cuenta ? cuenta.saldo : 0),
    desde: desde,
    saldoAnterior,
    hayAnteriores: previos.length > 0,
    dias: visibles.map((d) => ({
      fecha: d.fecha,
      rotulo: rotuloCorto(d.fecha),
      entregados: d.entregados,
      envios: d.envios,
      cobrado: num(d.cobrado),
      tarifas: num(d.tarifas),
      debeTraer: num(d.debeTraer),
      saldoDelDia: num(d.saldoDelDia),
      acumulado: num(d.acumulado),
      movimientos: (d.movimientos || []).map((m) => ({
        // Sin `id`: desde acá no se toca nada, así que no hace falta con qué apuntarle a una fila.
        verbo: MOVIMIENTO_LABEL[claseDelMovimiento(m.monto)] || '',
        monto: Math.abs(num(m.monto)),
        nota: m.nota || null,
        autor: m.autor || null,
        anulado: !movimientoVivo(m),
      })),
    })),
  };
}
 ...
 *
 * 🔴 **El sello de la hora se pega SÓLO a las acciones que mueven el estado.** `cobrado` y
 * `no_cobrado` no tocan si el paquete se entregó —son sobre la plata— y meterles la hora les haría
 * pisar el sello de la entrega real con el momento en que alguien tildó otra cosa. Por eso se
 * pregunta por `parche.estado` y no se aplica a todos por comodidad.
 *
 * 🔑 **La hora la pone el SERVIDOR** (`ahora`), no el teléfono: el reloj del celular del cadete
 * puede estar corrido, y una hora de entrega es un dato que después nadie puede corregir.
 */
export function parcheDeAccion(accion, ahora) {
  if (!Object.prototype.hasOwnProperty.call(ACCIONES, accion)) return null;
  const parche = { ...ACCIONES[accion] };
  if (parche.estado) Object.assign(parche, selloDeEntrega(parche.estado, ahora));
  return parche;
}
