...
import { CLAVES_PUERTA, PUERTAS, puertasDeMarca, rotuloPuerta } from './puertas.core.js';
...
export const PLANTILLAS = [
  {
    ...
    eje: {
      ...
      /*
        🆕 🔑 **Qué puertas existen en cada MARCA** (1-sep-2026). Es el único eje que lo tiene, y por
        eso es una función opcional del catálogo y ⛔ no un campo obligatorio de todos: el origen de
        una sesión de fotos y el qué-cambió de una condición comercial son los mismos en los dos
        negocios. Quien lo lee es `clavesDeEje`, y con eso la pantalla y el servidor cortan por la
        MISMA lista.

        Bruno, 1-sep-2026: *«bdi y zattia tienen compra nacional; la diferencia es que bdi tiene
        importado, y zattia tiene producción propia»*.
      */
      clavesEn: (marca) => puertasDeMarca(marca).map((x) => x.key),
      rotulo: rotuloPuerta,
...
/**
 * **Los valores del eje que existen en esta marca.** Sin `clavesEn` son todos: es el caso de los
 * ejes que no dependen del negocio (el origen de una sesión, el qué-cambió de una condición).
 *
 * 🔑 **Una sola implementación para la pantalla y para el servidor.** El modal dibuja esta lista y
 * `sembrar` la usa como guard: si cada uno cortara por su lado, la pantalla ofrecería una opción
 * que el servidor rechaza —o peor, al revés—. Es la misma razón por la que `pendientesDe` es la
 * función del badge y de la lista.
 */
export function clavesDeEje(eje, marca) {
  if (!eje) return [];
  return typeof eje.clavesEn === 'function' ? eje.clavesEn(marca) : eje.claves;
}

/**
 * ¿Este valor del eje existe en esta marca? Es un guard **aparte** de `eje.claves.includes(...)`:
 * aquél contesta *«ese valor no existe»* y éste *«ese valor no existe **acá**»*, que son dos cosas
 * distintas para quien lee el error — y la segunda dice qué hacer.
 */
export function ejeValeEnMarca(eje, valor, marca) {
  return clavesDeEje(eje, marca).includes(String(valor));
}
