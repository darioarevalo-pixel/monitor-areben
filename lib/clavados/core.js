...
export function resumirClavados(renglones) {
  ...
  for (const r of filas) {
    ...
    if (r.parado == null) {
      ...
      parado += r.parado
      // 🔴 El recupero de los medibles va aparte, y ésta es la línea que arregla el defecto que
      // encontró caminar prod el 24-ago-2026: con UN solo clavado sin costo, `parado` sumaba 0 y el
      // porcentaje daba **100 % recuperado** — el número exacto que este archivo existe para no
      // mostrar. Un producto cuyo denominador no se conoce **no puede aportar al numerador**: si lo
      // hace, cada clavado sin costo empuja el porcentaje hacia arriba.
      recuperadoMedible += num(r.recuperado)
    }
  }
  return {
    productos: filas.length,
    agotados: filas.filter((r) => r.agotado).length,
    /** Lo recuperado por TODOS: es plata que volvió de verdad, se pueda medir el resto o no. */
    recuperado,
    parado,
    /** Cuántos productos no tienen costo legible. `parado` y `pct` **no los incluyen**. */
    sinCosto,
    /** ⛔ `null` cuando no hay ni un producto medible: un porcentaje sin denominador es un invento. */
    pct: parado + recuperadoMedible > 0 ? (recuperadoMedible / (parado + recuperadoMedible)) * 100 : null,
  }
}
