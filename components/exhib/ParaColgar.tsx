 ...
 * «traé la variante 9».
 *
 * 🔴 **Y ⛔ NO dice cuántas unidades hay** (21-sep-2026). Bruno, leyendo el primer mandado real:
 * *«no me tiene que decir son 309 items o algo así, porque no me interesa el stock del local, me
 * interesa que se exhiba»*. La tarea es **colgar UNA** de cada color que ⛔ no está colgado; que el
 * sistema tenga 10 en el guardado ⛔ no la cambia, y el número invitaba a traer las 10. Es la misma
 * regla que el mandado del balance (`filasBuscar`): **la cuenta es la cantidad de renglones**.
 *
...
export function ParaColgar({ lista, titulo, archivo, plegable }: { lista: Colgar[]; titulo?: string; archivo?: string; plegable?: boolean }) {
  const { variantes, productos } = resumenColgar(lista)
      ...
      <div style={{ maxHeight: plegable ? 260 : 420, overflowY: 'auto', margin: `${space[2]}px 0` }}>
        {grupos.map((g) => (
          <div key={g.productId} style={{ padding: '7px 2px', borderBottom: `1px solid ${color.line}` }}>
            <div style={{ fontWeight: 600, fontSize: font.base, color: color.ink }}>{g.name}</div>
            <div style={{ fontSize: font.xs, color: color.mut }}>
              {g.variantes.map((c) => c.it.size || '—').join(' · ')} · está en «{g.lugar}»
            </div>
  ...
  return (
    <Notice tone="warning" icon="🧺" style={{ marginBottom: space[4] }}>
      <div style={{ fontWeight: 700 }}>
        {titulo || 'Para colgar'}: {variantes} {variantes === 1 ? 'prenda' : 'prendas'} de {productos} {productos === 1 ? 'modelo' : 'modelos'}
      </div>
