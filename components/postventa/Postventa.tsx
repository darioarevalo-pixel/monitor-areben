...
import {
  Button, SectionCard, Card, StatusPill, Field, Input, NumberField, Toolbar, Tabs, EmptyState, KpiCard,
    ...
    <div style={{ maxWidth: 1100 }}>
      ...
      {esAdmin && tab === 'cambios' ? (
        ...
        <>
          ...
          {!esAdmin && <div style={{ fontSize: font.sm, color: color.mut, marginBottom: space[3] }}>Cargá acá la prenda con falla que recibís del cliente. Administración la recibe y confirma.</div>}
          ...
          <Toolbar justify="between" style={{ margin: `${space[3]}px 0` }}>
            {esAdmin ? (
              ...
              <div style={{ fontSize: font.md, fontWeight: weight.bold, color: color.ink2 }}>📦 Pendientes de enviar a depósito{visibles.length ? ` (${visibles.length})` : ''}</div>
            )}
