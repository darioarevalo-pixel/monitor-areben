...
/**
 * La promo bancaria de hoy, arriba de la pantalla del mostrador.
 *
 ...
 *
 * No cuesta un fetch: el store de la agenda ya lo llenó el shell (`useAvisosPoll`), igual que las
 * novedades. Y **no se dibuja nada si hoy no hay promo** — un cartel que dice "hoy no hay" todos los
 * días entrena a saltear la zona donde el día que sí hay va a aparecer el aviso.
 */
...
export function BandaPromoHoy() {
  const { marca } = useSesion()
  const promos = useAgenda((s) => s.promos)
  const deHoy = promosDe(promos, hoyIso(), { canal: 'mostrador', marca })
