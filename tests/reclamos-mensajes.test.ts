...
describe('mensaje de apertura', () => {
  const txt = mensajeApertura({ ...base, motivo: 'falla' } as ReclamoRow, 'D-0025', 'https://x/reclamo/tok')
...
describe('mensaje de resolución', () => {
  it('devolución total: dice el monto exacto', () => {
    const t = mensajeResolucion({ ...base, compensacion: 'plata_total', monto_total: 15283, destino_prenda: 'stock', via_retorno: 'andreani' } as ReclamoRow, 'D-0025')
  ...
  it('si se la queda, lo dice explícitamente', () => {
    const t = mensajeResolucion({ ...base, compensacion: 'plata_parcial', monto_total: 5000, destino_prenda: 'falla', via_retorno: null } as ReclamoRow, 'D-0025')
    expect(t).toContain('No hace falta que nos devuelvas nada')
    expect(t).toContain('$ 5.000')
  })

  it('reenvío del faltante: no promete plata', () => {
    const t = mensajeResolucion({ ...base, compensacion: 'reenvio', monto_total: 0, destino_prenda: 'no_salio' } as ReclamoRow, 'D-0025')
    expect(t).toContain('Te enviamos lo que falta')
    expect(t).not.toContain('devolvemos $')
  })

  it('cupón: incluye el código', () => {
    const t = mensajeResolucion({ ...base, compensacion: 'cupon', cupon_codigo: 'ABC123', destino_prenda: 'falla' } as ReclamoRow, 'D-0025')
    expect(t).toContain('ABC123')
  })

  it('presencial: le dice que se acerque, no que espere una etiqueta', () => {
    const t = mensajeResolucion({ ...base, compensacion: 'plata_total', monto_total: 1000, via_retorno: 'presencial', destino_prenda: 'stock' } as ReclamoRow, 'D-0025')
  ...
  it('avisa que la acreditación tarda, sin prometer un plazo exacto', () => {
    const t = mensajeResolucion({ ...base, compensacion: 'plata_total', monto_total: 1000, destino_prenda: 'stock' } as ReclamoRow, 'D-0025')
...
describe('mensaje de seguimiento', () => {
  it('etiqueta: incluye el código y aclara quién paga', () => {
    const t = mensajeSeguimiento({ ...base, seguimiento_vuelta: 'AR123' } as ReclamoRow, 'D-0025', 'etiqueta')
    expect(t).toContain('AR123')
    expect(t).toContain('por nuestra cuenta')
  })

  it('reenvío: usa el seguimiento de IDA, no el de vuelta', () => {
    const t = mensajeSeguimiento({ ...base, seguimiento_ida: 'IDA9', seguimiento_vuelta: 'VUELTA1' } as ReclamoRow, 'D-0025', 'reenvio')
    expect(t).toContain('IDA9')
    expect(t).not.toContain('VUELTA1')
  })

  it('plata: dice el monto', () => {
    const t = mensajeSeguimiento({ ...base, monto_total: 15283 } as ReclamoRow, 'D-0025', 'plata')
    expect(t).toContain('$ 15.283')
  })
})
...
describe('resumenCorto', () => {
  it('arma la etiqueta del listado', () => {
    expect(resumenCorto({ motivo: 'mal_armado', cliente: 'carla ietta' } as ReclamoRow, 'D-0007'))
